...
func (bav *UtxoView) getMessage(messageHash *types.BlockHash) *types.PGMessage {
...
func (bav *UtxoView) setMessageMappings(message *types.PGMessage) {
	bav.MessageMap[*message.MessageHash] = message
}

func (bav *UtxoView) deleteMessageMappings(message *types.PGMessage) {
...
func (bav *UtxoView) _connectPrivateMessage(
	...
	if uint64(len(txMeta.EncryptedText)) > bav.Params.MaxPrivateMessageLengthBytes {
		return 0, 0, nil, errors.Wrapf(
			types.RuleErrorPrivateMessageEncryptedTextLengthExceedsMax, "_connectPrivateMessage: "+
	...
	if len(txMeta.RecipientPublicKey) != btcec.PubKeyBytesLenCompressed {
		return 0, 0, nil, errors.Wrapf(
			types.RuleErrorPrivateMessageRecipientPubKeyLen, "_connectPrivateMessage: "+
	...
	if err != nil {
		return 0, 0, nil, errors.Wrapf(
			types.RuleErrorPrivateMessageParsePubKeyError, "_connectPrivateMessage: Parse error: %v", err)
	}

	// You can't send a message to yourself.
	if reflect.DeepEqual(txn.PublicKey, txMeta.RecipientPublicKey) {
		return 0, 0, nil, errors.Wrapf(
			types.RuleErrorPrivateMessageSenderPublicKeyEqualsRecipientPublicKey,
	...
	if txMeta.TimestampNanos == 0 {
		return 0, 0, nil, types.RuleErrorPrivateMessageTstampIsZero
	}
	...
	if bav.Postgres == nil {
		...
		if senderMessage != nil && !senderMessage.isDeleted {
			return 0, 0, nil, errors.Wrapf(
				types.RuleErrorPrivateMessageExistsWithSenderPublicKeyTstampTuple,
		...
		if recipientMessage != nil && !recipientMessage.isDeleted {
			return 0, 0, nil, errors.Wrapf(
				types.RuleErrorPrivateMessageExistsWithRecipientPublicKeyTstampTuple,
	...
	if hasExtraV {
		Version, _ := network.Uvarint(extraV)
		messageEntry.Version = uint8(Version)
	}

	if bav.Postgres != nil {
		message := &types.PGMessage{
...
func (bav *UtxoView) _disconnectPrivateMessage(
	...
	txMeta := currentTxn.TxnMeta.(*network.PrivateMessageMetadata)
	...
	if !reflect.DeepEqual(messageEntry.SenderPublicKey, currentTxn.PublicKey) {
		return fmt.Errorf("_disconnectPrivateMessage: Sender public key on "+
			...
			types.PkToString(messageEntry.SenderPublicKey, bav.Params),
	...
	if !reflect.DeepEqual(messageEntry.RecipientPublicKey, txMeta.RecipientPublicKey) {
		return fmt.Errorf("_disconnectPrivateMessage: Recipient public key on "+
			...
			types.PkToString(messageEntry.RecipientPublicKey, bav.Params),
			types.PkToString(txMeta.RecipientPublicKey, bav.Params))
	}
