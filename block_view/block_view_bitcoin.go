...
func (bav *UtxoView) _disconnectBitcoinExchange(
	...
	// OperationTypeBitcoinExchange.
	txMeta := currentTxn.TxnMeta.(*network.BitcoinExchangeMetadata)
	...
	bitcoinTxHash := (types.BlockHash)(txMeta.BitcoinTransaction.TxHash())
...
func (bav *UtxoView) _disconnectUpdateBitcoinUSDExchangeRate(
	...
	// Get the transaction metadata from the transaction now that we know it has
	// OperationTypeUpdateBitcoinUSDExchangeRate.
	txMeta := currentTxn.TxnMeta.(*network.UpdateBitcoinUSDExchangeRateMetadataa)
	_ = txMeta
...
func (bav *UtxoView) _existsBitcoinTxIDMapping(bitcoinBurnTxID *types.BlockHash) bool {
...
func (bav *UtxoView) _setBitcoinBurnTxIDMappings(bitcoinBurnTxID *types.BlockHash) {
	bav.BitcoinBurnTxIDs[*bitcoinBurnTxID] = true
}

func (bav *UtxoView) _deleteBitcoinBurnTxIDMappings(bitcoinBurnTxID *types.BlockHash) {
...
func (bav *UtxoView) _connectBitcoinExchange(
	...
	if bav.Params.DeflationBombBlockHeight != 0 &&
		...
		return 0, 0, nil, types.RuleErrorDeflationBombForbidsMintingAnyMoreDeSo
	}
	...
	txMetaa := txn.TxnMeta.(*network.BitcoinExchangeMetadata)
	...
	if len(txn.TxInputs) != 0 {
		return 0, 0, nil, types.RuleErrorBitcoinExchangeShouldNotHaveInputs
	}
	if len(txn.TxOutputs) != 0 {
		return 0, 0, nil, types.RuleErrorBitcoinExchangeShouldNotHaveOutputs
	}
	if len(txn.PublicKey) != 0 {
		return 0, 0, nil, types.RuleErrorBitcoinExchangeShouldNotHavePublicKey
	}
	if txn.Signature != nil {
		return 0, 0, nil, types.RuleErrorBitcoinExchangeShouldNotHaveSignature
	}
	...
	bitcoinTxHash := (types.BlockHash)(txMetaa.BitcoinTransaction.TxHash())
	if bav._existsBitcoinTxIDMapping(&bitcoinTxHash) {
		return 0, 0, nil, types.RuleErrorBitcoinExchangeDoubleSpendingBitcoinTransaction
	}
	...
	if err != nil {
		return 0, 0, nil, types.RuleErrorBitcoinExchangeValidPublicKeyNotFoundInInputs
	}
	...
	if addrFromPubKey.AddressPubKeyHash().EncodeAddress() == bav.Params.BitcoinBurnAddress {
		return 0, 0, nil, types.RuleErrorBurnAddressCannotBurnBitcoin
	}
	...
	if err != nil {
		return 0, 0, nil, types.RuleErrorBitcoinExchangeProblemComputingBurnOutput
	}
	if totalBurnOutput <= 0 {
		return 0, 0, nil, types.RuleErrorBitcoinExchangeTotalOutputLessThanOrEqualZero
	}
	...
	nanosToCreate := types.CalcNanosToCreate(bav.NanosPurchased, uint64(totalBurnOutput), usdCentsPerBitcoin)
	...
	if feeNanosBigint.Cmp(big.NewInt(math.MaxInt64)) > 0 ||
		...
		return 0, 0, nil, types.RuleErrorBitcoinExchangeFeeOverflow
	}
...
func (bav *UtxoView) _connectUpdateBitcoinUSDExchangeRate(
	...
	// Check that the transaction has the right TxnType.
	if txn.TxnMeta.GetTxnType() != network.TxnTypeUpdateBitcoinUSDExchangeRate {
	...
	if txMeta.USDCentsPerBitcoin < types.MinUSDCentsPerBitcoin {
		return 0, 0, nil, types.RuleErrorExchangeRateTooLow
	}
	if txMeta.USDCentsPerBitcoin > types.MaxUSDCentsPerBitcoin {
	...
	if !updaterIsParamUpdater {
		return 0, 0, nil, types.RuleErrorUserNotAuthorizedToUpdateExchangeRate
	}
