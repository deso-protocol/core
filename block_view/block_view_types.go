...
func (utxoEntry *UtxoEntry) String() string {
	return fmt.Sprintf("< PublicKey: %v, BlockHeight: %d, AmountNanos: %d, UtxoType: %v, "+
		"isSpent: %v, utxoKey: %v>", types.PkToStringMainnet(utxoEntry.PublicKey),
...
type UsernameMapKey [types.MaxUsernameLengthBytes]byte
...
func (mm *MessageKey) String() string {
	return fmt.Sprintf("<Public Key: %s, TstampNanos: %d>",
		types.PkToStringMainnet(mm.PublicKey[:]), mm.TstampNanos)
}

// StringKey is useful for creating maps that need to be serialized to JSON.
func (mm *MessageKey) StringKey(params *types.DeSoParams) string {
	return types.PkToString(mm.PublicKey[:], params) + "_" + fmt.Sprint(mm.TstampNanos)
}
...
func MakeLikeKey(userPk []byte, LikedPostHash types.BlockHash) LikeKey {
	return LikeKey{
...
func MakeNFTKey(nftPostHash *types.BlockHash, serialNumber uint64) NFTKey {
	return NFTKey{
...
type NFTEntry struct {
	LastOwnerPKID              *types.PKID // This is needed to decrypt unlockable text.
	OwnerPKID                  *types.PKID
	NFTPostHash                *types.BlockHash
	SerialNumber               uint64
	IsForSale                  bool
...
func MakeNFTBidKey(bidderPKID *types.PKID, nftPostHash *types.BlockHash, serialNumber uint64) NFTBidKey {
	return NFTBidKey{
...
type DerivedKeyEntry struct {
	...
	// Derived public key
	DerivedPublicKey types.PublicKey
	...
	OperationType network.AuthorizeDerivedKeyOperationType
...
func MakeDerivedKeyMapKey(ownerPublicKey types.PublicKey, derivedPublicKey types.PublicKey) DerivedKeyMapKey {
...
func MakeFollowKey(followerPKID *types.PKID, followedPKID *types.PKID) FollowKey {
	return FollowKey{
...
type DiamondKey struct {
	...
	ReceiverPKID    types.PKID
	DiamondPostHash types.BlockHash
}

func MakeDiamondKey(senderPKID *types.PKID, receiverPKID *types.PKID, diamondPostHash *types.BlockHash) DiamondKey {
	return DiamondKey{
...
func (mm *DiamondKey) String() string {
	return fmt.Sprintf("<SenderPKID: %v, ReceiverPKID: %v, DiamondPostHash: %v>",
		types.PkToStringMainnet(mm.SenderPKID[:]), types.PkToStringMainnet(mm.ReceiverPKID[:]),
...
func MakeRepostKey(userPk []byte, RepostedPostHash types.BlockHash) RepostKey {
	return RepostKey{
...
type BalanceEntryMapKey struct {
	HODLerPKID  types.PKID
	CreatorPKID types.PKID
}

func MakeCreatorCoinBalanceKey(hodlerPKID *types.PKID, creatorPKID *types.PKID) BalanceEntryMapKey {
...
func (mm BalanceEntryMapKey) String() string {
	return fmt.Sprintf("BalanceEntryMapKey: <HODLer Pub Key: %v, Creator Pub Key: %v>",
		types.PkToStringBoth(mm.HODLerPKID[:]), types.PkToStringBoth(mm.CreatorPKID[:]))
}
...
func (pkid *PKIDEntry) String() string {
	return fmt.Sprintf("< PKID: %s, PublicKey: %s >", types.PkToStringMainnet(pkid.PKID[:]), types.PkToStringMainnet(pkid.PublicKey))
}
...
type UtxoView struct {
	...
	UtxoKeyToUtxoEntry          map[types.UtxoKey]*UtxoEntry
	PublicKeyToDeSoBalanceNanos map[types.PublicKey]uint64

	// BitcoinExchange data
	NanosPurchased     uint64
	USDCentsPerBitcoin uint64
	GlobalParamsEntry  *GlobalParamsEntry
	BitcoinBurnTxIDs   map[types.BlockHash]bool
	...
	MessageMap map[types.BlockHash]*types.PGMessage
	...
	// Post data
	PostHashToPostEntry map[types.BlockHash]*PostEntry

	// Profile data
	PublicKeyToPKIDEntry map[PkMapKey]*PKIDEntry
	// The PKIDEntry is only used here to store the public key.
	PKIDToPublicKey               map[types.PKID]*PKIDEntry
	ProfilePKIDToProfileEntry     map[types.PKID]*ProfileEntry
...
type UtxoOperation struct {
	...
	FounderRewardUtxoKey *types.UtxoKey
