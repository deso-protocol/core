...
func TestBasicTransfer(t *testing.T) {
	...
	// transaction-level public key should fail.
	{
		...
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorInputWithPublicKeyDifferentFromTxnPublicKey)
	}
	...
	// A block reward with a bad signature should fail.
	{
		...
		_, _, _, _, err =
			utxoView.ConnectTransaction(txn, txHash, getTxnSize(*txn), blockHeight,
				true /*verifySignature*/, false /*ignoreUtxos*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorBlockRewardTxnNotAllowedToHaveSignature)
	}
	...
	// should fail.
	{
		...
		_, _, _, _, err =
			utxoView.ConnectTransaction(txn, txHash, getTxnSize(*txn), blockHeight,
				true /*verifySignature*/, false /*ignoreUtxos*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorBlockRewardTxnNotAllowedToHaveInputs)
	}

	// A block with too much block reward should fail.
	allowedBlockReward := types.CalcBlockRewardNanos(chain.blockTip().Height)
	assert.Equal(int64(allowedBlockReward), int64(1*types.NanosPerUnit))
	...
	require.NoError(err)
	{
		...
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorBlockRewardExceedsMaxAllowed)
	}
