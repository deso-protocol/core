...
func TestFollowTxns(t *testing.T) {
	...
	require.Error(err)
	require.Contains(err.Error(), types.RuleErrorFollowingNonexistentProfile)
	...
	_, _, _, err = _doFollowTxn(
		...
		m1Pub, m0Priv, false /*isUnfollow*/)
	require.Error(err)
	require.Contains(err.Error(), types.RuleErrorFollowEntryAlreadyExists)
	...
	require.Error(err)
	require.Contains(err.Error(), types.RuleErrorCannotUnfollowNonexistentFollowEntry)
