...
func CalculateCreatorCoinToMintPolynomial(
	deltaDeSoNanos uint64, currentCreatorCoinSupplyNanos uint64, params *types.DeSoParams) uint64 {
	...
	bigNanosPerUnit := types.NewFloat().SetUint64(types.NanosPerUnit)
	bigDeltaDeSo := types.Div(types.NewFloat().SetUint64(deltaDeSoNanos), bigNanosPerUnit)
	bigCurrentCreatorCoinSupply :=
		types.Div(types.NewFloat().SetUint64(currentCreatorCoinSupplyNanos), bigNanosPerUnit)
	...
	bigRet := types.Sub(types.BigFloatPow((types.Div((types.Add(bigDeltaDeSo,
		types.Mul(params.CreatorCoinSlope, types.Mul(params.CreatorCoinReserveRatio,
			types.BigFloatPow(bigCurrentCreatorCoinSupply, (types.Div(types.bigOne,
				params.CreatorCoinReserveRatio))))))), types.Mul(params.CreatorCoinSlope,
	...
	retNanos, _ := types.Mul(bigRet, bigNanosPerUnit).Uint64()
	return retNanos
}

func CalculateCreatorCoinToMintBancor(
	deltaDeSoNanos uint64, currentCreatorCoinSupplyNanos uint64,
	currentDeSoLockedNanos uint64, params *types.DeSoParams) uint64 {
	// The values our equations take are generally in whole units rather than
	// nanos, so the first step is to convert the nano amounts into floats
	// representing full coin units.
	bigNanosPerUnit := types.NewFloat().SetUint64(types.NanosPerUnit)
	bigDeltaDeSo := types.Div(types.NewFloat().SetUint64(deltaDeSoNanos), bigNanosPerUnit)
	bigCurrentCreatorCoinSupply := types.Div(types.NewFloat().SetUint64(currentCreatorCoinSupplyNanos), bigNanosPerUnit)
	bigCurrentDeSoLocked := types.Div(types.NewFloat().SetUint64(currentDeSoLockedNanos), bigNanosPerUnit)
	...
	bigRet := types.Mul(bigCurrentCreatorCoinSupply,
		types.Sub(types.BigFloatPow((types.Add(types.bigOne, types.Div(bigDeltaDeSo,
			bigCurrentDeSoLocked))),
			(params.CreatorCoinReserveRatio)), types.bigOne))
	// The value we get is generally a number of whole creator coins, and so we
	// need to convert it to "nanos" as a last step.
	retNanos, _ := types.Mul(bigRet, bigNanosPerUnit).Uint64()
	return retNanos
...
func CalculateDeSoToReturn(
	...
	// representing full coin units.
	bigNanosPerUnit := types.NewFloat().SetUint64(types.NanosPerUnit)
	bigDeltaCreatorCoin := types.Div(types.NewFloat().SetUint64(deltaCreatorCoinNanos), bigNanosPerUnit)
	bigCurrentCreatorCoinSupply := types.Div(types.NewFloat().SetUint64(currentCreatorCoinSupplyNanos), bigNanosPerUnit)
	bigCurrentDeSoLocked := types.Div(types.NewFloat().SetUint64(currentDeSoLockedNanos), bigNanosPerUnit)
	...
	//
	// Sorry the code for the equation is so hard to read.
	bigRet := types.Mul(bigCurrentDeSoLocked, (types.Sub(types.bigOne, types.BigFloatPow((types.Sub(types.bigOne,
		types.Div(bigDeltaCreatorCoin, bigCurrentCreatorCoinSupply))), (types.Div(types.bigOne,
...
func CalculateCreatorCoinToMint(
	...
	params *types.DeSoParams) uint64 {
...
func (bav *UtxoView) HelpConnectCreatorCoinBuy(
	...
	if totalInput == 0 {
		return 0, 0, 0, 0, nil, types.RuleErrorCreatorCoinRequiresNonZeroInput
	}
	...
	if len(txMeta.ProfilePublicKey) != btcec.PubKeyBytesLenCompressed {
		return 0, 0, 0, 0, nil, types.RuleErrorCreatorCoinInvalidPubKeySize
	}
	...
	if existingProfileEntry == nil || existingProfileEntry.isDeleted {
		return 0, 0, 0, 0, nil, errors.Wrapf(
			types.RuleErrorCreatorCoinOperationOnNonexistentProfile,
	...
	if desoBeforeFeesNanos == 0 {
		return 0, 0, 0, 0, nil, types.RuleErrorCreatorCoinBuyMustTradeNonZeroDeSo
	}
	...
	if totalOutput > math.MaxUint64-desoBeforeFeesNanos {
		return 0, 0, 0, 0, nil, errors.Wrapf(
			types.RuleErrorCreatorCoinTxnOutputWithInvalidBuyAmount,
	...
	if totalInput < totalOutput {
		return 0, 0, 0, 0, nil, errors.Wrapf(
			types.RuleErrorCreatorCoinTxnOutputExceedsInput,
	...
	desoAfterFeesNanos := types.IntDiv(
		types.IntMul(
	...
	if desoAfterFeesNanos == 0 {
		return 0, 0, 0, 0, nil, types.RuleErrorCreatorCoinBuyMustTradeNonZeroDeSoAfterFees
	}
	...
	if blockHeight > types.DeSoFounderRewardBlockHeight &&
		...
		desoFounderRewardNanos = types.IntDiv(
			types.IntMul(
	...
	if desoRemainingNanos == 0 {
		return 0, 0, 0, 0, nil, types.RuleErrorCreatorCoinBuyMustTradeNonZeroDeSoAfterFounderReward
	}
	...
	if blockHeight > types.SalomonFixBlockHeight {
		if creatorCoinToMintNanos < bav.Params.CreatorCoinAutoSellThresholdNanos {
			return 0, 0, 0, 0, nil, types.RuleErrorCreatorCoinBuyMustSatisfyAutoSellThresholdNanos
		}
	}
	...
	if blockHeight > types.DeSoFounderRewardBlockHeight {
	...
	} else if blockHeight > types.SalomonFixBlockHeight {
		...
		creatorCoinFounderRewardNanos = types.IntDiv(
			types.IntMul(
	...
	if txMeta.MinCreatorCoinExpectedNanos != 0 &&
		...
		return 0, 0, 0, 0, nil, errors.Wrapf(
			types.RuleErrorCreatorCoinLessThanMinimumSetByUser,
	...
	if buyerBalanceEntry == nil ||
		(buyerBalanceEntry.isDeleted && blockHeight > types.BuyCreatorCoinAfterDeletedBalanceEntryFixBlockHeight) {
	...
	if reflect.DeepEqual(txn.PublicKey, existingProfileEntry.PublicKey) {
		...
		if creatorBalanceEntry == nil ||
			(creatorBalanceEntry.isDeleted && blockHeight > types.BuyCreatorCoinAfterDeletedBalanceEntryFixBlockHeight) {
	...
	if blockHeight > types.SalomonFixBlockHeight {
		if buyerBalanceEntry.BalanceNanos == 0 && coinsBuyerGetsNanos != 0 &&
			...
			return 0, 0, 0, 0, nil, types.RuleErrorCreatorCoinBuyMustSatisfyAutoSellThresholdNanosForBuyer
		}
	}
	...
	if creatorBalanceEntry.BalanceNanos == 0 &&
		...
		blockHeight > types.SalomonFixBlockHeight {

		return 0, 0, 0, 0, nil, types.RuleErrorCreatorCoinBuyMustSatisfyAutoSellThresholdNanosForCreator
	}
...
func (bav *UtxoView) HelpConnectCreatorCoinSell(
	...
	// to force-convert all your creator coin into DeSo. Think about it.
	if totalInput == 0 {
		return 0, 0, 0, nil, types.RuleErrorCreatorCoinRequiresNonZeroInput
	}
	...
	txMeta := txn.TxnMeta.(*network.CreatorCoinMetadataa)
	if len(txMeta.ProfilePublicKey) != btcec.PubKeyBytesLenCompressed {
		return 0, 0, 0, nil, types.RuleErrorCreatorCoinInvalidPubKeySize
	}
	...
	if sellerBalanceEntry == nil || sellerBalanceEntry.isDeleted {
		return 0, 0, 0, nil, types.RuleErrorCreatorCoinSellerBalanceEntryDoesNotExist
	}
	...
	if creatorCoinToSellNanos == 0 {
		return 0, 0, 0, nil, types.RuleErrorCreatorCoinSellMustTradeNonZeroCreatorCoin
	}
	...
	if creatorCoinToSellNanos > sellerBalanceEntry.BalanceNanos {
		return 0, 0, 0, nil, errors.Wrapf(
			types.RuleErrorCreatorCoinSellInsufficientCoins,
	...
	if existingProfileEntry.DeSoLockedNanos == 0 {
		return 0, 0, 0, nil, types.RuleErrorCreatorCoinSellNotAllowedWhenZeroDeSoLocked
	}
	...
	if txMeta.MinDeSoExpectedNanos != 0 &&
		...
		return 0, 0, 0, nil, errors.Wrapf(
			types.RuleErrorDeSoReceivedIsLessThanMinimumSetBySeller,
...
func (bav *UtxoView) _connectCreatorCoin(
	...
	case network.CreatorCoinOperationTypeBuy:
	...
	case network.CreatorCoinOperationTypeSell:
	...
	case network.CreatorCoinOperationTypeAddDeSo:
...
func (bav *UtxoView) _disconnectCreatorCoin(
	...
	txMeta := currentTxn.TxnMeta.(*network.CreatorCoinMetadataa)
	...
	if txMeta.OperationType == network.CreatorCoinOperationTypeBuy {
		...
		if !reflect.DeepEqual(currentTxn.PublicKey, txMeta.ProfilePublicKey) {
			...
			founderRewardNanos := types.IntDiv(
				types.IntMul(
		...
		*transactorBalanceEntry = *operationData.PrevTransactorBalanceEntry
		...
		// transaction.
	} else if txMeta.OperationType == network.CreatorCoinOperationTypeSell {
	...
	} else if txMeta.OperationType == network.CreatorCoinOperationTypeAddDeSo {
...
func (bav *UtxoView) _setBalanceEntryMappingsWithPKIDs(
	balanceEntry *BalanceEntry, hodlerPKID *types.PKID, creatorPKID *types.PKID) {
...
func (bav *UtxoView) GetBalanceEntryForHODLerPubKeyAndCreatorPubKey(
	...
	_balanceEntry *BalanceEntry, _hodlerPKID *types.PKID, _creatorPKID *types.PKID) {
...
func (bav *UtxoView) GetHoldings(pkid *types.PKID, fetchProfiles bool) ([]*BalanceEntry, []*ProfileEntry, error) {
	...
	holdingsMap := make(map[types.PKID]*BalanceEntry)
...
func (bav *UtxoView) GetHolders(pkid *types.PKID, fetchProfiles bool) ([]*BalanceEntry, []*ProfileEntry, error) {
	...
	holdersMap := make(map[types.PKID]*BalanceEntry)
...
func (bav *UtxoView) _getBalanceEntryForHODLerPKIDAndCreatorPKID(
	hodlerPKID *types.PKID, creatorPKID *types.PKID) *BalanceEntry {
