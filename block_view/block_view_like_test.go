...
func TestLikeTxns(t *testing.T) {
	...
	doLikeTxn := func(
		senderPkBase58Check string, likedPostHash types.BlockHash,
	...
	require.Error(err)
	require.Contains(err.Error(), types.RuleErrorCannotLikeNonexistentPost)
	...
	require.Error(err)
	require.Contains(err.Error(), types.RuleErrorLikeEntryAlreadyExists)
	...
	m0Likes := []types.BlockHash{
		post1Hash,
	}

	m1Likes := []types.BlockHash{
		post2Hash,
	}

	m2Likes := []types.BlockHash{
		post1Hash,
		post3Hash,
	}

	m3Likes := []types.BlockHash{
		post1Hash,
		post2Hash,
	}
	...
	require.Error(err)
	require.Contains(err.Error(), types.RuleErrorCannotUnlikeWithoutAnExistingLike)
