...
func (bav *UtxoView) GetNFTEntryForNFTKey(nftKey *NFTKey) *NFTEntry {
	...
	if bav.Postgres != nil {
		...
		nftEntry = db.DBGetNFTEntryByPostHashSerialNumber(bav.Handle, &nftKey.NFTPostHash, nftKey.SerialNumber)
	}
...
func (bav *UtxoView) GetNFTEntriesForPostHash(nftPostHash *types.BlockHash) []*NFTEntry {
	...
	if bav.Postgres != nil {
		...
		dbNFTEntries = db.DBGetNFTEntriesForPostHash(bav.Handle, nftPostHash)
	}
...
func (bav *UtxoView) GetNFTEntriesForPKID(ownerPKID *types.PKID) []*NFTEntry {
	var dbNFTEntries []*NFTEntry
	if bav.Postgres != nil {
		...
		dbNFTEntries = db.DBGetNFTEntriesForPKID(bav.Handle, ownerPKID)
	}
...
func (bav *UtxoView) GetNFTBidEntriesForPKID(bidderPKID *types.PKID) (_nftBidEntries []*NFTBidEntry) {
	...
	if bav.Postgres != nil {
		...
		dbNFTBidEntries = db.DBGetNFTBidEntriesForPKID(bav.Handle, bidderPKID)
	}
...
// TODO: Postgres
func (bav *UtxoView) GetHighAndLowBidsForNFTSerialNumber(nftHash *types.BlockHash, serialNumber uint64) (_highBid uint64, _lowBid uint64) {
	highBid := uint64(0)
	lowBid := uint64(0)
...
// TODO: Postgres
func (bav *UtxoView) GetDBHighAndLowBidsForNFT(nftHash *types.BlockHash, serialNumber uint64) (_highBid uint64, _lowBid uint64) {
...
func (bav *UtxoView) GetDBHighAndLowBidEntriesForNFT(
	nftHash *types.BlockHash, serialNumber uint64,
	...
	exitLoop := false
	highBidEntries := db.DBGetNFTBidEntriesPaginated(
	...
	exitLoop = false
	lowBidEntries := db.DBGetNFTBidEntriesPaginated(
...
func (bav *UtxoView) GetAcceptNFTBidHistoryForNFTKey(nftKey *NFTKey) *[]*NFTBidEntry {
	...
	// defer to the db. If a mapping exists in the db, return it. If not, return
	// nil.
	dbNFTBidEntries := db.DBGetAcceptedNFTBidEntriesByPostHashSerialNumber(bav.Handle, &nftKey.NFTPostHash, nftKey.SerialNumber)
...
func (bav *UtxoView) GetNFTBidEntryForNFTBidKey(nftBidKey *NFTBidKey) *NFTBidEntry {
	...
	if bav.Postgres != nil {
		...
		dbNFTBidEntry = db.DBGetNFTBidEntryForNFTBidKey(bav.Handle, nftBidKey)
	}
...
func (bav *UtxoView) GetAllNFTBidEntries(nftPostHash *types.BlockHash, serialNumber uint64) []*NFTBidEntry {
	// Get all the entries in the DB.
	...
	if bav.Postgres != nil {
		...
		dbEntries = db.DBGetNFTBidEntries(bav.Handle, nftPostHash, serialNumber)
	}
...
func (bav *UtxoView) _connectNFTBid(
	...
	txMeta := txn.TxnMeta.(*network.NFTBidMetadata)
	...
	if postEntry == nil || postEntry.isDeleted {
		return 0, 0, nil, types.RuleErrorNFTBidOnNonExistentPost
	} else if !postEntry.IsNFT {
		return 0, 0, nil, types.RuleErrorNFTBidOnPostThatIsNotAnNFT
	} else if txMeta.SerialNumber > postEntry.NumNFTCopies {
		return 0, 0, nil, types.RuleErrorNFTBidOnInvalidSerialNumber
	}
	...
	if txMeta.SerialNumber != uint64(0) {
		...
		if txMeta.BidAmountNanos < nftEntry.MinBidAmountNanos && !(txMeta.BidAmountNanos == 0 && prevNFTBidEntry != nil) {
			return 0, 0, nil, types.RuleErrorNFTBidLessThanMinBidAmountNanos
		}
	}
	...
	} else if txMeta.BidAmountNanos > spendableBalance && blockHeight > types.BrokenNFTBidsFixBlockHeight {
		return 0, 0, nil, types.RuleErrorInsufficientFundsForNFTBid
	}

	// Force the input to be non-zero so that we can prevent replay attacks.
	if totalInput == 0 {
		return 0, 0, nil, types.RuleErrorNFTBidRequiresNonZeroInput
	}
...
func (bav *UtxoView) _disconnectNFTBid(
	...
	txMeta := currentTxn.TxnMeta.(*network.NFTBidMetadata)
...
func (bav *UtxoView) _connectAcceptNFTBid(
	...
	txMeta := txn.TxnMeta.(*network.AcceptNFTBidMetadata)
	...
	if prevNFTEntry == nil || prevNFTEntry.isDeleted {
		...
		return 0, 0, nil, errors.Wrapf(types.RuleErrorNFTBidOnNonExistentNFTEntry, "_connectAcceptNFTBid: ")
	}
	if !prevNFTEntry.IsForSale {
		return 0, 0, nil, errors.Wrapf(types.RuleErrorNFTBidOnNFTThatIsNotForSale, "_connectAcceptNFTBid: ")
	}

	// Verify the NFT is not a pending transfer.
	if prevNFTEntry.IsPending {
		return 0, 0, nil, types.RuleErrorCannotAcceptBidForPendingNFTTransfer
	}
	...
	if updaterPKID == nil || updaterPKID.isDeleted {
		return 0, 0, nil, fmt.Errorf("_connectAcceptNFTBid: non-existent updaterPKID: %s",
			types.PkToString(txn.PublicKey, bav.Params))
	}
	if !reflect.DeepEqual(prevNFTEntry.OwnerPKID, updaterPKID.PKID) {
		return 0, 0, nil, types.RuleErrorAcceptNFTBidByNonOwner
	}
	...
	if nftPostEntry == nil || nftPostEntry.isDeleted {
		return 0, 0, nil, types.RuleErrorPostEntryNotFoundForAcceptedNFTBid
	}
	if nftPostEntry.HasUnlockable && len(txMeta.UnlockableText) == 0 {
		return 0, 0, nil, types.RuleErrorUnlockableNFTMustProvideUnlockableText
	}

	// Check the length of the UnlockableText.
	if uint64(len(txMeta.UnlockableText)) > bav.Params.MaxPrivateMessageLengthBytes {
		return 0, 0, nil, errors.Wrapf(
			types.RuleErrorUnlockableTextLengthExceedsMax, "_connectAcceptNFTBid: "+
	...
	if existingProfileEntry == nil || existingProfileEntry.isDeleted {
		return 0, 0, nil, fmt.Errorf(
			...
			types.PkToStringMainnet(nftPostEntry.PosterPublicKey), types.PkToStringTestnet(nftPostEntry.PosterPublicKey))
	}
	...
	if nftBidEntry == nil || nftBidEntry.isDeleted {
		...
		nftBidEntry = bav.GetNFTBidEntryForNFTBidKey(&nftBidKey)
		if nftBidEntry == nil || nftBidEntry.isDeleted {
			return 0, 0, nil, types.RuleErrorCantAcceptNonExistentBid
		}
	}
	if nftBidEntry.BidAmountNanos != txMeta.BidAmountNanos {
		return 0, 0, nil, types.RuleErrorAcceptedNFTBidAmountDoesNotMatch
	}
	...
	if err != nil {
		return 0, 0, nil, fmt.Errorf(
			...
			types.PkToStringBoth(txn.PublicKey))
	}
	...
	if err != nil {
		return 0, 0, nil, fmt.Errorf(
			...
			types.PkToStringBoth(bidderPublicKey))
	}
	...
	if err != nil {
		return 0, 0, nil, fmt.Errorf(
			...
			types.PkToStringBoth(nftPostEntry.PosterPublicKey))
	}
	...
	if len(txMeta.BidderInputs) == 0 {
		return 0, 0, nil, types.RuleErrorAcceptedNFTBidMustSpecifyBidderInputs
	}
	...
	for _, bidderInput := range txMeta.BidderInputs {
		bidderUtxoKey := types.UtxoKey(*bidderInput)
		bidderUtxoEntry := bav.GetUtxoEntryForUtxoKey(&bidderUtxoKey)
		if bidderUtxoEntry == nil || bidderUtxoEntry.isSpent {
			return 0, 0, nil, types.RuleErrorBidderInputForAcceptedNFTBidNoLongerExists
		}
	...
	if totalBidderInput < txMeta.BidAmountNanos {
		return 0, 0, nil, types.RuleErrorAcceptNFTBidderInputsInsufficientForBidAmount
	}
	...
	creatorRoyaltyNanos := types.IntDiv(
		types.IntMul(
	...
	creatorCoinRoyaltyNanos := types.IntDiv(
		types.IntMul(
			big.NewInt(int64(txMeta.BidAmountNanos)),
	...
	// Force the input to be non-zero so that we can prevent replay attacks.
	if totalInput == 0 {
		return 0, 0, nil, types.RuleErrorAcceptNFTBidRequiresNonZeroInput
	}
	...
	nftPaymentUtxoKeys := []*types.UtxoKey{}
	nextUtxoIndex := uint32(len(txn.TxOutputs))
	sellerOutputKey := &types.UtxoKey{
		TxID:  *txHash,
		Index: nextUtxoIndex,
	}
	...
	if creatorRoyaltyNanos > 0 {
		nextUtxoIndex += 1
		royaltyOutputKey := &types.UtxoKey{
			TxID:  *txHash,
			Index: nextUtxoIndex,
		}
	...
	if bidderChangeNanos > 0 {
		nextUtxoIndex += 1
		bidderChangeOutputKey := &types.UtxoKey{
			TxID:  *txHash,
			Index: nextUtxoIndex,
		}
...
func (bav *UtxoView) _disconnectAcceptNFTBid(
	...
	txMeta := currentTxn.TxnMeta.(*network.AcceptNFTBidMetadata)
	operationData := utxoOpsForTxn[operationIndex]
	operationIndex--
...
func (bav *UtxoView) _connectBurnNFT(
	...
	if blockHeight < types.NFTTransferOrBurnAndDerivedKeysBlockHeight {
		return 0, 0, nil, types.RuleErrorBurnNFTBeforeBlockHeight
	}

	// Check that the transaction has the right TxnType.
	if txn.TxnMeta.GetTxnType() != network.TxnTypeBurnNFT {
		return 0, 0, nil, fmt.Errorf("_connectBurnNFT: called with bad TxnType %s",
			txn.TxnMeta.GetTxnType().String())
	}
	txMeta := txn.TxnMeta.(*network.BurnNFTMetadata)

	// Verify the NFT entry exists.
	nftKey := MakeNFTKey(txMeta.NFTPostHash, txMeta.SerialNumber)
	nftEntry := bav.GetNFTEntryForNFTKey(&nftKey)
	if nftEntry == nil || nftEntry.isDeleted {
		return 0, 0, nil, types.RuleErrorCannotBurnNonExistentNFT
	}
	...
	if !reflect.DeepEqual(nftEntry.OwnerPKID, updaterPKID.PKID) {
		return 0, 0, nil, types.RuleErrorBurnNFTByNonOwner
	}

	// Verify that the NFT is not for sale.
	if nftEntry.IsForSale {
		return 0, 0, nil, types.RuleErrorCannotBurnNFTThatIsForSale
	}
	...
	// Force the input to be non-zero so that we can prevent replay attacks.
	if totalInput == 0 {
		return 0, 0, nil, types.RuleErrorBurnNFTRequiresNonZeroInput
	}
...
func (bav *UtxoView) _disconnectBurnNFT(
	...
	txMeta := currentTxn.TxnMeta.(*network.BurnNFTMetadata)
	operationData := utxoOpsForTxn[operationIndex]
	operationIndex--
	...
	if updaterPKID == nil || updaterPKID.isDeleted {
		return fmt.Errorf("_disconnectBurnNFT: non-existent updaterPKID: %s",
			types.PkToString(currentTxn.PublicKey, bav.Params))
	}
	if !reflect.DeepEqual(operationData.PrevNFTEntry.OwnerPKID, updaterPKID.PKID) {
		return fmt.Errorf("_disconnectBurnNFT: updaterPKID does not match NFT owner: %s, %s",
			types.PkToString(updaterPKID.PKID[:], bav.Params),
			types.PkToString(operationData.PrevNFTEntry.OwnerPKID[:], bav.Params))
	}
...
func (bav *UtxoView) _connectCreateNFT(
	...
	// Check that the transaction has the right TxnType.
	if txn.TxnMeta.GetTxnType() != network.TxnTypeCreateNFT {
		return 0, 0, nil, fmt.Errorf("_connectCreateNFT: called with bad TxnType %s",
			txn.TxnMeta.GetTxnType().String())
	}
	txMeta := txn.TxnMeta.(*network.CreateNFTMetadata)

	// Validate the txMeta.
	if txMeta.NumCopies > bav.GlobalParamsEntry.MaxCopiesPerNFT {
		return 0, 0, nil, types.RuleErrorTooManyNFTCopies
	}
	if txMeta.NumCopies == 0 {
		return 0, 0, nil, types.RuleErrorNFTMustHaveNonZeroCopies
	}
	...
	if math.MaxUint64-txMeta.NFTRoyaltyToCreatorBasisPoints < txMeta.NFTRoyaltyToCoinBasisPoints {
		return 0, 0, nil, types.RuleErrorNFTRoyaltyOverflow
	}
	...
	if royaltyBasisPoints > bav.Params.MaxNFTRoyaltyBasisPoints {
		return 0, 0, nil, types.RuleErrorNFTRoyaltyHasTooManyBasisPoints
	}
	postEntry := bav.GetPostEntryForPostHash(txMeta.NFTPostHash)
	if postEntry == nil || postEntry.isDeleted {
		return 0, 0, nil, types.RuleErrorCreateNFTOnNonexistentPost
	}
	if IsVanillaRepost(postEntry) {
		return 0, 0, nil, types.RuleErrorCreateNFTOnVanillaRepost
	}
	if !reflect.DeepEqual(postEntry.PosterPublicKey, txn.PublicKey) {
		return 0, 0, nil, types.RuleErrorCreateNFTMustBeCalledByPoster
	}
	if postEntry.IsNFT {
		return 0, 0, nil, types.RuleErrorCreateNFTOnPostThatAlreadyIsNFT
	}
	profileEntry := bav.GetProfileEntryForPublicKey(postEntry.PosterPublicKey)
	if profileEntry == nil || profileEntry.isDeleted {
		return 0, 0, nil, types.RuleErrorCantCreateNFTWithoutProfileEntry
	}
	...
	// Force the input to be non-zero so that we can prevent replay attacks.
	if totalInput == 0 {
		return 0, 0, nil, types.RuleErrorCreateNFTRequiresNonZeroInput
	}
	...
	if totalInput < totalOutput {
		return 0, 0, nil, types.RuleErrorCreateNFTWithInsufficientFunds
	}
	...
	*prevPostEntry = *postEntry
	...
	if posterPKID == nil || posterPKID.isDeleted {
		return 0, 0, nil, fmt.Errorf("_connectCreateNFT: non-existent posterPKID: %s",
			types.PkToString(postEntry.PosterPublicKey, bav.Params))
	}
...
func (bav *UtxoView) _disconnectCreateNFT(
	...
	txMeta := currentTxn.TxnMeta.(*network.CreateNFTMetadata)
	operationData := utxoOpsForTxn[operationIndex]
	operationIndex--
...
func (bav *UtxoView) _connectNFTTransfer(
	...
	prevNFTEntry := bav.GetNFTEntryForNFTKey(&nftKey)
	if prevNFTEntry == nil || prevNFTEntry.isDeleted {
		return 0, 0, nil, types.RuleErrorCannotTransferNonExistentNFT
	}
	...
	if !reflect.DeepEqual(prevNFTEntry.OwnerPKID, updaterPKID.PKID) {
		return 0, 0, nil, types.RuleErrorNFTTransferByNonOwner
	}
	...
	if receiverPKID == nil || receiverPKID.isDeleted {
		return 0, 0, nil, fmt.Errorf(
			"_connectNFTTransfer: Found nil or deleted PKID for receiver, this should never "+
				"happen. Receiver pubkey: %v", types.PkToStringMainnet(txMeta.ReceiverPublicKey))
	}

	// Make sure that the NFT entry is not for sale.
	if prevNFTEntry.IsForSale {
		return 0, 0, nil, types.RuleErrorCannotTransferForSaleNFT
	}
	...
	if nftPostEntry.HasUnlockable && len(txMeta.UnlockableText) == 0 {
		return 0, 0, nil, types.RuleErrorCannotTransferUnlockableNFTWithoutUnlockable
	}

	// Check the length of the UnlockableText.
	if uint64(len(txMeta.UnlockableText)) > bav.Params.MaxPrivateMessageLengthBytes {
		return 0, 0, nil, errors.Wrapf(
			types.RuleErrorUnlockableTextLengthExceedsMax, "_connectNFTTransfer: "+
				"UnlockableTextLen = %d; Max length = %d",
			len(txMeta.UnlockableText), bav.Params.MaxPrivateMessageLengthBytes)
	}
	...
	// Force the input to be non-zero so that we can prevent replay attacks.
	if totalInput == 0 {
		return 0, 0, nil, types.RuleErrorNFTTransferRequiresNonZeroInput
	}
...
func (bav *UtxoView) _disconnectNFTTransfer(
	...
	txMeta := currentTxn.TxnMeta.(*network.NFTTransferMetadata)
	operationData := utxoOpsForTxn[operationIndex]
	operationIndex--
...
func (bav *UtxoView) _connectAcceptNFTTransfer(
	...
	if blockHeight < types.NFTTransferOrBurnAndDerivedKeysBlockHeight {
		return 0, 0, nil, types.RuleErrorAcceptNFTTransferBeforeBlockHeight
	}

	// Check that the transaction has the right TxnType.
	if txn.TxnMeta.GetTxnType() != network.TxnTypeAcceptNFTTransfer {
		return 0, 0, nil, fmt.Errorf("_connectAcceptNFTTransfer: called with bad TxnType %s",
			txn.TxnMeta.GetTxnType().String())
	}
	txMeta := txn.TxnMeta.(*network.AcceptNFTTransferMetadata)

	// Verify the NFT entry exists.
	nftKey := MakeNFTKey(txMeta.NFTPostHash, txMeta.SerialNumber)
	prevNFTEntry := bav.GetNFTEntryForNFTKey(&nftKey)
	if prevNFTEntry == nil || prevNFTEntry.isDeleted {
		return 0, 0, nil, types.RuleErrorCannotAcceptTransferOfNonExistentNFT
	}
	...
	if !reflect.DeepEqual(prevNFTEntry.OwnerPKID, updaterPKID.PKID) {
		return 0, 0, nil, types.RuleErrorAcceptNFTTransferByNonOwner
	}

	// Verify that the NFT is actually pending.
	if !prevNFTEntry.IsPending {
		return 0, 0, nil, types.RuleErrorAcceptNFTTransferForNonPendingNFT
	}
	...
	// Force the input to be non-zero so that we can prevent replay attacks.
	if totalInput == 0 {
		return 0, 0, nil, types.RuleErrorAcceptNFTTransferRequiresNonZeroInput
	}
...
func (bav *UtxoView) _disconnectAcceptNFTTransfer(
	...
	txMeta := currentTxn.TxnMeta.(*network.AcceptNFTTransferMetadata)
	operationData := utxoOpsForTxn[operationIndex]
	operationIndex--
...
func (bav *UtxoView) _connectUpdateNFT(
	...
	// Check that the transaction has the right TxnType.
	if txn.TxnMeta.GetTxnType() != network.TxnTypeUpdateNFT {
		return 0, 0, nil, fmt.Errorf("_connectUpdateNFT: called with bad TxnType %s",
			txn.TxnMeta.GetTxnType().String())
	}
	txMeta := txn.TxnMeta.(*network.UpdateNFTMetadata)

	// Verify the NFT entry exists.
	nftKey := MakeNFTKey(txMeta.NFTPostHash, txMeta.SerialNumber)
	prevNFTEntry := bav.GetNFTEntryForNFTKey(&nftKey)
	if prevNFTEntry == nil || prevNFTEntry.isDeleted {
		return 0, 0, nil, types.RuleErrorCannotUpdateNonExistentNFT
	}

	// Verify the NFT is not a pending transfer.
	if prevNFTEntry.IsPending {
		return 0, 0, nil, types.RuleErrorCannotUpdatePendingNFTTransfer
	}
	...
	if !reflect.DeepEqual(prevNFTEntry.OwnerPKID, updaterPKID.PKID) {
		return 0, 0, nil, types.RuleErrorUpdateNFTByNonOwner
	}
	...
	if prevNFTEntry.IsForSale == txMeta.IsForSale {
		return 0, 0, nil, types.RuleErrorNFTUpdateMustUpdateIsForSaleStatus
	}
	...
	// Force the input to be non-zero so that we can prevent replay attacks.
	if totalInput == 0 {
		return 0, 0, nil, types.RuleErrorUpdateNFTRequiresNonZeroInput
	}
...
func (bav *UtxoView) _disconnectUpdateNFT(
	...
	txMeta := currentTxn.TxnMeta.(*network.UpdateNFTMetadata)
	operationData := utxoOpsForTxn[operationIndex]
	operationIndex--
