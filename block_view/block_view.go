...
func (bav *UtxoView) _ResetViewMappingsAfterFlush() {
	// Utxo data
	bav.UtxoKeyToUtxoEntry = make(map[types.UtxoKey]*UtxoEntry)
	// TODO: Deprecate this value
	bav.NumUtxoEntries = db.GetUtxoNumEntries(bav.Handle)
	bav.PublicKeyToDeSoBalanceNanos = make(map[types.PublicKey]uint64)
	...
	bav.BitcoinBurnTxIDs = make(map[types.BlockHash]bool)
	...
	bav.PostHashToPostEntry = make(map[types.BlockHash]*PostEntry)
	bav.PublicKeyToPKIDEntry = make(map[PkMapKey]*PKIDEntry)
	bav.PKIDToPublicKey = make(map[types.PKID]*PKIDEntry)
	bav.ProfilePKIDToProfileEntry = make(map[types.PKID]*ProfileEntry)
	...
	bav.MessageMap = make(map[types.BlockHash]*types.PGMessage)
...
func (bav *UtxoView) CopyUtxoView() (*UtxoView, error) {
	...
	newView.UtxoKeyToUtxoEntry = make(map[types.UtxoKey]*UtxoEntry, len(bav.UtxoKeyToUtxoEntry))
	...
	newView.PublicKeyToDeSoBalanceNanos = make(map[types.PublicKey]uint64, len(bav.PublicKeyToDeSoBalanceNanos))
	...
	newView.BitcoinBurnTxIDs = make(map[types.BlockHash]bool, len(bav.BitcoinBurnTxIDs))
	...
	// Copy the post data
	newView.PostHashToPostEntry = make(map[types.BlockHash]*PostEntry, len(bav.PostHashToPostEntry))
	...
	newView.PKIDToPublicKey = make(map[types.PKID]*PKIDEntry, len(bav.PKIDToPublicKey))
	...
	newView.ProfilePKIDToProfileEntry = make(map[types.PKID]*ProfileEntry, len(bav.ProfilePKIDToProfileEntry))
	...
	newView.MessageMap = make(map[types.BlockHash]*types.PGMessage, len(bav.MessageMap))
...
func NewUtxoView(
	...
	if view.Postgres != nil {
		view.TipHash = view.Postgres.GetChain(types.MAIN_CHAIN).TipHash
	} else {
...
func (bav *UtxoView) GetUtxoEntryForUtxoKey(utxoKey *types.UtxoKey) *UtxoEntry {
...
func (bav *UtxoView) GetDeSoBalanceNanosForPublicKey(publicKey []byte) (uint64, error) {
	balanceNanos, hasBalance := bav.PublicKeyToDeSoBalanceNanos[*types.NewPublicKey(publicKey)]
	if hasBalance {
	...
	if bav.Postgres != nil {
		balanceNanos = bav.Postgres.GetBalance(types.NewPublicKey(publicKey))
	} else {
		var err error
	...
	bav.PublicKeyToDeSoBalanceNanos[*types.NewPublicKey(publicKey)] = balanceNanos
...
func (bav *UtxoView) _unSpendUtxo(utxoEntryy *UtxoEntry) error {
	...
	bav.PublicKeyToDeSoBalanceNanos[*types.NewPublicKey(utxoEntryy.PublicKey)] = desoBalanceNanos

	return nil
}

func (bav *UtxoView) _spendUtxo(utxoKey *types.UtxoKey) (*UtxoOperation, error) {
	...
	bav.PublicKeyToDeSoBalanceNanos[*types.NewPublicKey(utxoEntry.PublicKey)] = desoBalanceNanos
...
func (bav *UtxoView) _unAddUtxo(utxoKey *types.UtxoKey) error {
	// Get the entry for this utxo from the view if it's cached,
	// otherwise try and get it from the db.
	utxoEntry := bav.GetUtxoEntryForUtxoKey(utxoKey)
	if utxoEntry == nil {
...
func (bav *UtxoView) DisconnectTransaction(currentTxn *network.MsgDeSoTxn, txnHash *types.BlockHash,
	utxoOpsForTxn []*UtxoOperation, blockHeight uint32) error {

	if currentTxn.TxnMeta.GetTxnType() == network.TxnTypeBlockReward || currentTxn.TxnMeta.GetTxnType() == network.TxnTypeBasicTransfer {
		return bav._disconnectBasicTransfer(
			currentTxn, txnHash, utxoOpsForTxn, blockHeight)

	} else if currentTxn.TxnMeta.GetTxnType() == network.TxnTypeBitcoinExchange {
		return bav._disconnectBitcoinExchange(
			OperationTypeBitcoinExchange, currentTxn, txnHash, utxoOpsForTxn, blockHeight)

	} else if currentTxn.TxnMeta.GetTxnType() == network.TxnTypePrivateMessage {
		return bav._disconnectPrivateMessage(
			OperationTypePrivateMessage, currentTxn, txnHash, utxoOpsForTxn, blockHeight)

	} else if currentTxn.TxnMeta.GetTxnType() == network.TxnTypeSubmitPost {
		return bav._disconnectSubmitPost(
			OperationTypeSubmitPost, currentTxn, txnHash, utxoOpsForTxn, blockHeight)

	} else if currentTxn.TxnMeta.GetTxnType() == network.TxnTypeUpdateProfile {
		return bav._disconnectUpdateProfile(
			OperationTypeUpdateProfile, currentTxn, txnHash, utxoOpsForTxn, blockHeight)

	} else if currentTxn.TxnMeta.GetTxnType() == network.TxnTypeUpdateBitcoinUSDExchangeRate {
...
func (bav *UtxoView) DisconnectBlock(
	desoBlock *network.MsgDeSoBlock, txHashes []*types.BlockHash, utxoOps [][]*UtxoOperation) error {
	...
	for _, txn := range desoBlock.Txns {
		...
		if txn.TxnMeta.GetTxnType() == network.TxnTypeAcceptNFTBid {
			numInputs += len(txn.TxnMeta.(*network.AcceptNFTBidMetadata).BidderInputs)
		}
...
func _isEntryImmatureBlockReward(utxoEntry *UtxoEntry, blockHeight uint32, params *types.DeSoParams) bool {
...
func (bav *UtxoView) ConnectTransaction(txn *network.MsgDeSoTxn, txHash *types.BlockHash,
	txnSizeBytes int64,
...
func (bav *UtxoView) _connectTransaction(txn *network.MsgDeSoTxn, txHash *types.BlockHash,
	...
	if err := lib.CheckTransactionSanity(txn); err != nil {
	...
	if len(txnBytes) > int(bav.Params.MaxBlockSizeBytes/2) {
		return nil, 0, 0, 0, types.RuleErrorTxnTooBig
	}

	var totalInput, totalOutput uint64
	var utxoOpsForTxn []*UtxoOperation
	if txn.TxnMeta.GetTxnType() == network.TxnTypeBlockReward || txn.TxnMeta.GetTxnType() == network.TxnTypeBasicTransfer {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectBasicTransfer(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeBitcoinExchange {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectBitcoinExchange(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypePrivateMessage {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectPrivateMessage(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeSubmitPost {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectSubmitPost(
				txn, txHash, blockHeight, verifySignatures, ignoreUtxos)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeUpdateProfile {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectUpdateProfile(
				txn, txHash, blockHeight, verifySignatures, ignoreUtxos)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeUpdateBitcoinUSDExchangeRate {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectUpdateBitcoinUSDExchangeRate(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeUpdateGlobalParams {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectUpdateGlobalParams(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeFollow {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectFollow(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeLike {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectLike(txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeCreatorCoin {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectCreatorCoin(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeCreatorCoinTransfer {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectCreatorCoinTransfer(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeSwapIdentity {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectSwapIdentity(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeCreateNFT {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectCreateNFT(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeUpdateNFT {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectUpdateNFT(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeAcceptNFTBid {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectAcceptNFTBid(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeNFTBid {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectNFTBid(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeNFTTransfer {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectNFTTransfer(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeAcceptNFTTransfer {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectAcceptNFTTransfer(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeBurnNFT {
		totalInput, totalOutput, utxoOpsForTxn, err =
			bav._connectBurnNFT(
				txn, txHash, blockHeight, verifySignatures)

	} else if txn.TxnMeta.GetTxnType() == network.TxnTypeAuthorizeDerivedKey {
		totalInput, totalOutput, utxoOpsForTxn, err =
	...
	if txn.TxnMeta.GetTxnType() != network.TxnTypeBlockReward {
		...
		if totalInput < totalOutput {
			return nil, 0, 0, 0, types.RuleErrorTxnOutputExceedsInput
		}
	...
	if txn.TxnMeta.GetTxnType() != network.TxnTypeBitcoinExchange && txnSizeBytes != 0 && bav.GlobalParamsEntry.MinimumNetworkFeeNanosPerKB != 0 {
		// Make sure there isn't overflow in the fee.
		if fees != ((fees * 1000) / 1000) {
			return nil, 0, 0, 0, types.RuleErrorOverflowDetectedInFeeRateCalculation
		}
		...
		if (fees*1000)/uint64(txnSizeBytes) < bav.GlobalParamsEntry.MinimumNetworkFeeNanosPerKB {
			return nil, 0, 0, 0, types.RuleErrorTxnFeeBelowNetworkMinimum
		}
	}
...
func (bav *UtxoView) ConnectBlock(
	desoBlock *network.MsgDeSoBlock, txHashes []*types.BlockHash, verifySignatures bool, eventManager *lib.EventManager) (
	[][]*UtxoOperation, error) {
	...
	for txIndex, txn := range desoBlock.Txns {
		...
		if totalFees > (math.MaxUint64 - currentFees) {
			return nil, types.RuleErrorTxnOutputWithInvalidAmount
		}
		...
		if eventManager != nil {
			eventManager.transactionConnected(&lib.TransactionEvent{
				Txn:      txn,
				TxnHash:  txHash,
				UtxoView: bav,
	...
	for _, bro := range desoBlock.Txns[0].TxOutputs {
		if bro.AmountNanos > types.MaxNanos ||
			blockRewardOutput > (math.MaxUint64-bro.AmountNanos) {

			return nil, types.RuleErrorBlockRewardOutputWithInvalidAmount
		}
		blockRewardOutput += bro.AmountNanos
	}
	// Verify that the block reward does not overflow when added to
	// the block's fees.
	blockReward := types.CalcBlockRewardNanos(uint32(blockHeader.Height))
	if totalFees > types.MaxNanos ||
		blockReward > (math.MaxUint64-totalFees) {

		return nil, types.RuleErrorBlockRewardOverflow
	}
	...
	if blockRewardOutput > maxBlockReward {
		...
		return nil, types.RuleErrorBlockRewardExceedsMaxAllowed
	}
...
func (bav *UtxoView) Preload(desoBlock *network.MsgDeSoBlock) error {
	...
	for _, txn := range desoBlock.Txns {
		if txn.TxnMeta.GetTxnType() == network.TxnTypeFollow {
			...
			publicKeys = append(publicKeys, types.NewPublicKey(txn.PublicKey))
			publicKeys = append(publicKeys, types.NewPublicKey(txnMeta.FollowedPublicKey))
		} else if txn.TxnMeta.GetTxnType() == network.TxnTypeCreatorCoin {
			txnMeta := txn.TxnMeta.(*network.CreatorCoinMetadataa)
			publicKeys = append(publicKeys, types.NewPublicKey(txn.PublicKey))
			publicKeys = append(publicKeys, types.NewPublicKey(txnMeta.ProfilePublicKey))
		} else if txn.TxnMeta.GetTxnType() == network.TxnTypeUpdateProfile {
			publicKeys = append(publicKeys, types.NewPublicKey(txn.PublicKey))
		}
	}
	...
	if len(publicKeys) > 0 {
		for _, publicKey := range publicKeys {
			...
			pkidEntry := &PKIDEntry{
				PKID:      types.PublicKeyToPKID(publicKeyBytes),
	...
	var outputs []*types.PGTransactionOutput
	var follows []*types.PGFollow
	var balances []*types.PGCreatorCoinBalance
	var likes []*types.PGLike
	var posts []*types.PGPost
	var lowercaseUsernames []string

	for _, txn := range desoBlock.Txns {
		// Preload all the inputs
		for _, txInput := range txn.TxInputs {
			output := &types.PGTransactionOutput{
		...
		if txn.TxnMeta.GetTxnType() == network.TxnTypeFollow {
			...
			balance := &types.PGCreatorCoinBalance{
			...
			txnMeta := txn.TxnMeta.(*network.LikeMetadata)
			like := &types.PGLike{
			...
			txnMeta := txn.TxnMeta.(*network.SubmitPostMetadata)

			var postHash *types.BlockHash
			if len(txnMeta.PostHashToModify) != 0 {
				postHash = types.NewBlockHash(txnMeta.PostHashToModify)
			} else {
				postHash = txn.Hash()
			}

			posts = append(posts, &types.PGPost{
				PostHash: postHash,
			})
			...
			txnMeta := txn.TxnMeta.(*network.UpdateProfileMetadata)
...
func IsRestrictedPubKey(userGraylistState []byte, userBlacklistState []byte, moderationType string) bool {
	if moderationType == "unrestricted" {
		return false
	} else if reflect.DeepEqual(userBlacklistState, types.IsBlacklisted) {
		return true
	} else if moderationType == "leaderboard" && reflect.DeepEqual(userGraylistState, types.IsGraylisted) {
		return true
	} else {
		return false
	}
...
func (bav *UtxoView) GetUnspentUtxoEntrysForPublicKey(pkBytes []byte) ([]*UtxoEntry, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "UtxoView.GetUnspentUtxoEntrysForPublicKey: Problem fetching "+
			"utxos for public key %s", types.PkToString(pkBytes, bav.Params))
	}
...
func (bav *UtxoView) GetSpendableDeSoBalanceNanosForPublicKey(pkBytes []byte,
	...
	if bav.Postgres != nil {
		...
		for ii := uint64(1); ii < uint64(numImmatureBlocks); ii++ {
			...
			if types.GenesisBlockHashHex == nextBlockHash.String() {
				break
			}
			...
			if err != nil {
				return uint64(0), errors.Wrapf(
					err, "GetSpendableDeSoBalanceNanosForPublicKey: Problem getting block reward for "+
						"public key %s blockhash %s", types.PkToString(pkBytes, bav.Params), nextBlockHash.String())
			}
			immatureBlockRewards += blockRewardForPK
			if blockNode.Parent != nil {
				nextBlockHash = blockNode.Parent.Hash
			} else {
				nextBlockHash = types.GenesisBlockHash
			}
		}
	}
