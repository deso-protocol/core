...
func TestUpdateGlobalParams(t *testing.T) {
	...
	_, _, _ = _doBasicTransferWithViewFlush(
		...
		lib.moneyPrivString, 10*types.NanosPerUnit /*amount to send*/, 11 /*feerate*/)
	...
	// Should fail when founder key is not equal to moneyPk
	{
		...
		_, _, _, err := _updateGlobalParamsEntry(
			...
			false)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorUserNotAuthorizedToUpdateGlobalParams)
	}

	// Should pass when founder key is equal to moneyPk
	var updateGlobalParamsTxn *network.MsgDeSoTxn
	var err error
	{
