...
func TestSubmitPost(t *testing.T) {
	...
	// Creating a post from an unregistered profile should succeed
	{
		submitPost(
			...
			[]byte{}, /*parentStakeID*/
			&network.DeSoBodySchema{Body: "m0 post body 1 no profile"}, /*body*/
			[]byte{},
...

	{
		submitPost(
			...
			[]byte{}, /*postHashToModify*/
			[]byte{}, /*parentStakeID*/
			&network.DeSoBodySchema{Body: "m0 post body 2 no profile"}, /*body*/
			[]byte{},
...

	{
		submitPost(
			...
			[]byte{}, /*postHashToModify*/
			[]byte{}, /*parentStakeID*/
			&network.DeSoBodySchema{Body: "m1 post body 1 no profile"}, /*body*/
			[]byte{},
	...
	// Creating a post from a registered profile should succeed
	{
		...
		submitPost(
			...
			[]byte{}, /*postHashToModify*/
			[]byte{}, /*parentStakeID*/
			&network.DeSoBodySchema{Body: "m2 post body 1 WITH profile"}, /*body*/
			[]byte{},
...

	{
		...
		submitPost(
			...
			[]byte{}, /*postHashToModify*/
			[]byte{}, /*parentStakeID*/
			&network.DeSoBodySchema{Body: "m3 post body 1 WITH profile"}, /*body*/
			[]byte{},
	...
	// Create another post for m2
	{
		submitPost(
			...
			[]byte{}, /*postHashToModify*/
			[]byte{}, /*parentStakeID*/
			&network.DeSoBodySchema{Body: "m2 post body 2 WITH profile"}, /*body*/
			[]byte{},
	...
	// A zero input post should fail
	{
		_, _, _, err := _submitPost(
			...
			[]byte{}, /*postHashToModify*/
			[]byte{}, /*parentStakeID*/
			&network.DeSoBodySchema{Body: "this is a post body"}, /*body*/
			[]byte{},
			1502947011*1e9, /*tstampNanos*/
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorTxnMustHaveAtLeastOneInput)
	}
	...
	// PostHashToModify with bad length
	{
		_, _, _, err := _submitPost(
			...
			m0Pub,                                 /*updaterPkBase58Check*/
			m0Priv,                                /*updaterPrivBase58Check*/
			db.RandomBytes(types.HashSizeBytes-1), /*postHashToModify*/
			[]byte{},                              /*parentStakeID*/
			&network.DeSoBodySchema{Body: "this is a post body"}, /*body*/
			[]byte{},
			...
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorSubmitPostInvalidPostHashToModify)
	}
	...
	// Setting PostHashToModify should fail for a non-existent post
	{
		_, _, _, err := _submitPost(
			...
			m0Pub,                               /*updaterPkBase58Check*/
			m0Priv,                              /*updaterPrivBase58Check*/
			db.RandomBytes(types.HashSizeBytes), /*postHashToModify*/
			[]byte{},                            /*parentStakeID*/
			&network.DeSoBodySchema{Body: "this is a post body"}, /*body*/
			[]byte{},
			1502947048*1e9, /*tstampNanos*/
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorSubmitPostModifyingNonexistentPost)
	}
	...
	// Bad length for parent stake id should fail
	{
		_, _, _, err := _submitPost(
			...
			m0Priv,                                /*updaterPrivBase58Check*/
			[]byte{},                              /*postHashToModify*/
			db.RandomBytes(types.HashSizeBytes-1), /*parentStakeID*/
			&network.DeSoBodySchema{Body: "this is a post body"}, /*body*/
			[]byte{},
			1502947048*1e9, /*tstampNanos*/
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorSubmitPostInvalidParentStakeIDLength)
	}
	...
	// Non-owner modifying post should fail
	{
		_, _, _, err := _submitPost(
			...
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorSubmitPostPostModificationNotAuthorized)
	}
	...
	// Zero timestamp should fail
	{
		_, _, _, err = _submitPost(
			...
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorSubmitPostTimestampIsZero)
	}
	...
	// should fail
	{
		_, _, _, err = _submitPost(
			...
			db.RandomBytes(types.HashSizeBytes),             /*parentStakeID*/
			&network.DeSoBodySchema{Body: "m0 post body 2"}, /*body*/
			[]byte{},
	...
	// should be ignored.
	{
		submitPost(
			...
			post1Hash[:],                        /*postHashToModify*/
			db.RandomBytes(types.HashSizeBytes), /*parentStakeID*/
			&network.DeSoBodySchema{Body: "m0 post body MODIFIED"}, /*body*/
			[]byte{},
	...
	// all non-body posts should be unchanged.
	{
		submitPost(
			...
			post4Hash[:],                        /*postHashToModify*/
			db.RandomBytes(types.HashSizeBytes), /*parentStakeID*/
			&network.DeSoBodySchema{Body: "m2 post body MODIFIED"}, /*body*/
			[]byte{},
	...
	// ParamUpdater modifying their own post should succeed
	{
		submitPost(
			...
			post5Hash[:],                        /*postHashToModify*/
			db.RandomBytes(types.HashSizeBytes), /*parentStakeID*/
			&network.DeSoBodySchema{Body: "paramUpdater post body MODIFIED"}, /*body*/
			[]byte{},
	...
	// Modifying a post and then modifying it back should work.
	{
		submitPost(
			...
			post3Hash[:],                        /*postHashToModify*/
			db.RandomBytes(types.HashSizeBytes), /*parentStakeID*/
			&network.DeSoBodySchema{Body: "sldkfjlskdfjlajflkasjdflkasjdf"}, /*body*/
			[]byte{},
		...
		submitPost(
			...
			post3Hash[:],                        /*postHashToModify*/
			db.RandomBytes(types.HashSizeBytes), /*parentStakeID*/
			&network.DeSoBodySchema{Body: "m1 post body 1 no profile modified back"}, /*body*/
			[]byte{},
			1502947049*1e9, /*tstampNanos*/
			false /*isHidden*/)
	}
	...
	// Comment on a post with an anonymous public key
	{
		submitPost(
			...
			&network.DeSoBodySchema{Body: "comment 1 from m0 on post3"}, /*body*/
			[]byte{},
	...
	// Make a few more comments.
	{
		submitPost(
			...
			&network.DeSoBodySchema{Body: "comment 2 from m0 on post3"}, /*body*/
			[]byte{},
...

	{
		submitPost(
			...
			[]byte{},     /*postHashToModify*/
			post6Hash[:], /*parentStakeID*/
			&network.DeSoBodySchema{Body: "comment 1 from m2 on post6"}, /*body*/
			[]byte{},
...

	{
		submitPost(
			...
			[]byte{},     /*postHashToModify*/
			post6Hash[:], /*parentStakeID*/
			&network.DeSoBodySchema{Body: "comment 1 from m3 on post6"}, /*body*/
			[]byte{},
	...
	var comment3HiddenTxnIndex int
	{
		...
		_, _, _, err = _submitPost(
			...
			[]byte{},        /*parentStakeID*/
			&network.DeSoBodySchema{Body: "modifying comment 1 by m1 should fail"}, /*body*/
			[]byte{},
			1502947049*1e9, /*tstampNanos*/
			false /*isHidden*/)
		require.Error(err)
		...
		submitPost(
			...
			comment1Hash[:], /*postHashToModify*/
			[]byte{},        /*parentStakeID*/
			&network.DeSoBodySchema{Body: "comment from m0 on post3 MODIFIED"}, /*body*/
			[]byte{},
			1502947049*1e9, /*tstampNanos*/
			false /*isHidden*/)
		...
		submitPost(
			...
			[]byte{},        /*parentStakeID*/
			&network.DeSoBodySchema{Body: "comment from m2 on post6 MODIFIED"}, /*body*/
			[]byte{},
			1502947049*1e9, /*tstampNanos*/
			true /*isHidden*/)
	...
	// a profile actually exists for that stake ID.
	{
		submitPost(
			...
			&network.DeSoBodySchema{Body: "comment m0 on profile m1 [1]"}, /*body*/
			[]byte{},
	...
	comment5Hash := comment5Txn.Hash()
	{
		submitPost(
			...
			&network.DeSoBodySchema{Body: "comment m1 on profile m2 [1]"}, /*body*/
			[]byte{},
	...
	comment6Hash := comment6Txn.Hash()
	{
		submitPost(
			...
			&network.DeSoBodySchema{Body: "comment m3 on profile m3 [1]"}, /*body*/
			[]byte{},
	...
	comment7Hash := comment7Txn.Hash()
	{
		submitPost(
			...
			[]byte{},  /*postHashToModify*/
			m3PkBytes, /*parentStakeID*/
			&network.DeSoBodySchema{Body: "comment m0 on profile m3 [2]"}, /*body*/
			[]byte{},
	...
	// and fail when it's not.
	{
		submitPost(
			...
			[]byte{},        /*parentStakeID*/
			&network.DeSoBodySchema{Body: "comment 1 from m0 on post3 MODIFIED"}, /*body*/
			[]byte{},
			1502947049*1e9, /*tstampNanos*/
			true /*isHidden*/)
		...
		submitPost(
			...
			[]byte{},        /*parentStakeID*/
			&network.DeSoBodySchema{Body: "comment m1 on profile m2 [1] MODIFIED"}, /*body*/
			[]byte{},
			1502947049*1e9, /*tstampNanos*/
			true /*isHidden*/)
		submitPost(
	...
	// repost 3 - Quote Repost
	{
		submitPost(
			...
			[]byte{}, /*postHashToModify*/
			[]byte{}, /*parentStakeID*/
			&network.DeSoBodySchema{Body: "quote-post"},
			post5Hash[:],
	...
	// repost 4 - Quote Repost + hide
	{
		submitPost(
			...
			[]byte{}, /*postHashToModify*/
			[]byte{}, /*parentStakeID*/
			&network.DeSoBodySchema{Body: "quote-post-hide-me"},
			post6Hash[:],
	...
	{
		{
			...
			require.Error(err)
			require.Contains(err.Error(), types.RuleErrorSubmitPostRepostPostNotFound)
		}
		{
			...
			require.Error(err)
			require.Contains(err.Error(), types.RuleErrorSubmitPostRepostOfRepost)
		}
		{
			...
			require.Error(err)
			require.Contains(err.Error(), types.RuleErrorSubmitPostUpdateRepostHash)
		}
	...
	comparePostBody := func(postEntry *PostEntry, message string, repostPostHash *types.BlockHash) {
	...
	for ii := 0; ii < len(txnOps); ii++ {
		...
		require.Equal(expectedSenderBalances[backwardIter], lib._getBalance(t, chain, nil, types.PkToStringTestnet(currentTxn.PublicKey)))
	}
	...
	for ii, tx := range txns {
		...
		require.Equal(expectedSenderBalances[ii], lib._getBalance(t, chain, mempool, types.PkToStringTestnet(tx.PublicKey)))
...
func assertCommentCount(utxoView *UtxoView, require *require.Assertions, postHash *types.BlockHash,
...
func findPostByPostHash(posts []*PostEntry, targetPostHash *types.BlockHash) (_targetPost *PostEntry) {
