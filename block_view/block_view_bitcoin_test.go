...
func TestBitcoinExchange(t *testing.T) {
	...
	types.InitialUSDCentsPerBitcoinExchangeRate = uint64(1350000)
	defer func() {
		types.InitialUSDCentsPerBitcoinExchangeRate = oldInitialUSDCentsPerBitcoinExchangeRate
	}()

	paramsTmp := types.DeSoTestnetParams
	...
	for ii, bitcoinExchangeTxn := range bitcoinExchangeTxns {
		txnMeta := bitcoinExchangeTxn.TxnMeta.(*network.BitcoinExchangeMetadata)
		...
		{
			hash1 := (types.BlockHash)(blockForTxn.BlockHash())
		...
		{
			hash1 := (types.BlockHash)(blockForTxn.Header.MerkleRoot)
		...
		{
			txHash := ((types.BlockHash)(txnMeta.BitcoinTransaction.TxHash()))
		...
		require.NoError(err)
		parsedBitcoinExchangeTxn := &network.MsgDeSoTxn{}
	...
	// Applying the full transaction with its merkle proof should work.
	{
		...
		require.Equal(
			mempoolTxRet.Tx.TxnMeta.(*network.BitcoinExchangeMetadata),
			burnTxn1.TxnMeta.(*network.BitcoinExchangeMetadata))
	}
	...
	// The mempool should be able to process a burn transaction directly.
	{
		...
		require.Equal(
			mempoolTxRet.Tx.TxnMeta.(*network.BitcoinExchangeMetadata),
			burnTxn2.TxnMeta.(*network.BitcoinExchangeMetadata))
	}
	...
	params.ParamUpdaterPublicKeys[MakePkMapKey(types.MustBase58CheckDecode(lib.moneyPkString))] = true
	...
	utxoOpsList := [][]*UtxoOperation{}
	{
		...
		fff := append([]*network.MsgDeSoTxn{}, bitcoinExchangeTxns[:rateUpdateIndex]...)
		fff = append(fff, nil)
	...
	desoPub1 := types.Base58CheckEncode(pkBytes1, false /*isPrivate*/, paramsCopy)
	priv1, _ := _privStringToKeys(t, BitcoinTestnetPriv1)
	desoPriv1 := types.Base58CheckEncode(priv1.Serialize(), true /*isPrivate*/, paramsCopy)
	desoPub2 := types.Base58CheckEncode(pkBytes2, false /*isPrivate*/, paramsCopy)
	priv2, _ := _privStringToKeys(t, BitcoinTestnetPriv2)
	desoPriv2 := types.Base58CheckEncode(priv2.Serialize(), true /*isPrivate*/, paramsCopy)
	desoPub3 := types.Base58CheckEncode(pkBytes3, false /*isPrivate*/, paramsCopy)
	priv3, _ := _privStringToKeys(t, BitcoinTestnetPriv3)
	desoPriv3 := types.Base58CheckEncode(priv3.Serialize(), true /*isPrivate*/, paramsCopy)
	{
...
func TestSpendOffOfUnminedTxnsBitcoinExchange(t *testing.T) {
	...
	{
		txnCopy := *burnTxn1
		txnCopy.TxnMeta = &network.BitcoinExchangeMetadata{
			BitcoinTransaction: txnCopy.TxnMeta.(*network.BitcoinExchangeMetadata).BitcoinTransaction,
	...
	// in all of them being added.
	{
		...
		for fakeIndex, burnTxn := range bitcoinExchangeTxns[2:] {
			...
			if realIndex == rateUpdateIndex {
				newUSDCentsPerBitcoin := int64(27000 * 100)
				updaterPkBytes, _, err := types.Base58CheckDecode(lib.moneyPkString)
				require.NoError(err)
...
func TestUpdateExchangeRate(t *testing.T) {
	...
	_, _, _ = _doBasicTransferWithViewFlush(
		...
		lib.moneyPrivString, 6*types.NanosPerUnit /*amount to send*/, 11 /*feerate*/)
	...
	// Should fail when founder key is not equal to moneyPk
	{
		...
		_, _, _, err := _updateUSDCentsPerBitcoinExchangeRate(
			...
			newUSDCentsPerBitcoin)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorUserNotAuthorizedToUpdateExchangeRate)
	}

	// Should pass when founder key is equal to moneyPk
	var updateExchangeRateTxn *network.MsgDeSoTxn
	var err error
	{
		newUSDCentsPerBitcoin := uint64(27000 * 100)
...
func GetTestParamsCopy(
	...
	paramss *types.DeSoParams, minBurnBlocks uint32,
) *types.DeSoParams {
	// Set the BitcoinExchange-related params to canned values.
	paramsCopy := *paramss
	headerHash := (types.BlockHash)(startHeader.BlockHash())
	paramsCopy.BitcoinStartBlockNode = types.NewBlockNode(
		nil,         /*ParentNode*/
		&headerHash, /*Hash*/
		startHeight,
		types.DifficultyBitsToHash(startHeader.Bits),
		...
		types.StatusBitcoinHeaderValidated,
	)
...
func _privStringToKeys(t *testing.T, privString string) (*btcec.PrivateKey, *btcec.PublicKey) {
	require := require.New(t)
	result, _, err := types.Base58CheckDecodePrefix(privString, 1)
	require.NoError(err)
...
func _readBitcoinExchangeTestData(t *testing.T) (
	...
	blocks := []*wire.MsgBlock{}
	{
		...
		for _, ll := range lines {
			...
			blockHash := types.mustDecodeHexBlockHash(cols[0])
			block := &wire.MsgBlock{}
			...
			parsedBlockHash := (types.BlockHash)(block.BlockHash())
	...
	headerHeights := []uint32{}
	{
		...
		for _, ll := range lines {
			...
			require.Equal(headerHash, (types.BlockHash)(header.BlockHash()))
