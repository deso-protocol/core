...
func TestUpdateProfile(t *testing.T) {
	...
	types.ParamUpdaterProfileUpdateFixBlockHeight = 0
	types.UpdateProfileFixBlockHeight = 0
	...
	// Username too long should fail.
	{
		...
		_, _, _, err = _updateProfile(
			...
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorProfileUsernameTooLong)
	}
	...
	// Description too long should fail.
	{
		...
		_, _, _, err = _updateProfile(
			...
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorProfileDescriptionTooLong)
	}
	...
	// Profile pic too long should fail.
	{
		_, _, _, err = _updateProfile(
			...
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorMaxProfilePicSize)
	}
	...
	// Stake multiple too large should fail long too long should fail.
	{
		_, _, _, err = _updateProfile(
			...
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorProfileStakeMultipleSize)
	}
	...
	// Creator percentage too large should fail.
	{
		_, _, _, err = _updateProfile(
			...
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorProfileCreatorPercentageSize)
	}
	...
	// Invalid profile public key should fail.
	{
		...
		require.Contains(err.Error(), types.RuleErrorProfileBadPublicKey)
	}
	...
	// Profile public key that is not authorized should fail.
	{
		_, _, _, err = _updateProfile(
			...
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorProfilePubKeyNotAuthorized)
	}
	...
	// Username that does not match our regex should fail
	{
		_, _, _, err = _updateProfile(
			...
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorInvalidUsername)
	...
	// Trying to take an already-registered username should fail.
	{
		_, _, _, err = _updateProfile(
			...
			false /*isHidden*/)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorProfileUsernameExists)
	...
	// Create Profile Fee and Minimum Network Fee tests
	{
		...
		_, _, _, err = _updateProfile(
			...
			false)
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorCreateProfileTxnOutputExceedsInput)
		...
		require.Error(err)
		require.Contains(err.Error(), types.RuleErrorTxnFeeBelowNetworkMinimum)
	...
	for ii, tx := range txns {
		...
		require.Equal(expectedSenderBalances[ii], lib._getBalance(
			t, chain, mempool, types.PkToStringTestnet(tx.PublicKey)))
...
func TestSpamUpdateProfile(t *testing.T) {
	...
	for ii := 0; ii < numTxns; ii++ {
		...
		moneyPkBytes, _, _ := types.Base58CheckDecode(lib.moneyPkString)
