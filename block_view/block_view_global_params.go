...
func (bav *UtxoView) _connectUpdateGlobalParams(
	...
	if txn.TxnMeta.GetTxnType() != network.TxnTypeUpdateGlobalParams {
	...
	if !updaterIsParamUpdater {
		return 0, 0, nil, types.RuleErrorUserNotAuthorizedToUpdateGlobalParams
	}
	if len(extraData[types.USDCentsPerBitcoinKey]) > 0 {
		// Validate that the exchange rate is not less than the floor as a sanity-check.
		newUSDCentsPerBitcoin, usdCentsPerBitcoinBytesRead := network.Uvarint(extraData[types.USDCentsPerBitcoinKey])
		...
		if newUSDCentsPerBitcoin < types.MinUSDCentsPerBitcoin {
			return 0, 0, nil, types.RuleErrorExchangeRateTooLow
		}
		if newUSDCentsPerBitcoin > types.MaxUSDCentsPerBitcoin {
			return 0, 0, nil, types.RuleErrorExchangeRateTooHigh
		}
		newGlobalParamsEntry.USDCentsPerBitcoin = newUSDCentsPerBitcoin
	}

	if len(extraData[types.MinNetworkFeeNanosPerKBKey]) > 0 {
		newMinNetworkFeeNanosPerKB, minNetworkFeeNanosPerKBBytesRead := network.Uvarint(extraData[types.MinNetworkFeeNanosPerKBKey])
		...
		if newMinNetworkFeeNanosPerKB < types.MinNetworkFeeNanosPerKBValue {
			return 0, 0, nil, types.RuleErrorMinNetworkFeeTooLow
		}
		if newMinNetworkFeeNanosPerKB > types.MaxNetworkFeeNanosPerKBValue {
			return 0, 0, nil, types.RuleErrorMinNetworkFeeTooHigh
		}
		newGlobalParamsEntry.MinimumNetworkFeeNanosPerKB = newMinNetworkFeeNanosPerKB
	}

	if len(extraData[types.CreateProfileFeeNanosKey]) > 0 {
		newCreateProfileFeeNanos, createProfileFeeNanosBytesRead := network.Uvarint(extraData[types.CreateProfileFeeNanosKey])
		...
		if newCreateProfileFeeNanos < types.MinCreateProfileFeeNanos {
			return 0, 0, nil, types.RuleErrorCreateProfileFeeTooLow
		}
		if newCreateProfileFeeNanos > types.MaxCreateProfileFeeNanos {
			return 0, 0, nil, types.RuleErrorCreateProfileTooHigh
		}
		newGlobalParamsEntry.CreateProfileFeeNanos = newCreateProfileFeeNanos
	}

	if len(extraData[types.CreateNFTFeeNanosKey]) > 0 {
		newCreateNFTFeeNanos, createNFTFeeNanosBytesRead := network.Uvarint(extraData[types.CreateNFTFeeNanosKey])
		...
		if newCreateNFTFeeNanos < types.MinCreateNFTFeeNanos {
			return 0, 0, nil, types.RuleErrorCreateNFTFeeTooLow
		}
		if newCreateNFTFeeNanos > types.MaxCreateNFTFeeNanos {
			return 0, 0, nil, types.RuleErrorCreateNFTFeeTooHigh
		}
		newGlobalParamsEntry.CreateNFTFeeNanos = newCreateNFTFeeNanos
	}

	if len(extraData[types.MaxCopiesPerNFTKey]) > 0 {
		newMaxCopiesPerNFT, maxCopiesPerNFTBytesRead := network.Uvarint(extraData[types.MaxCopiesPerNFTKey])
		...
		if newMaxCopiesPerNFT < types.MinMaxCopiesPerNFT {
			return 0, 0, nil, types.RuleErrorMaxCopiesPerNFTTooLow
		}
		if newMaxCopiesPerNFT > types.MaxMaxCopiesPerNFT {
			return 0, 0, nil, types.RuleErrorMaxCopiesPerNFTTooHigh
		}
	...
	if _, exists := extraData[types.ForbiddenBlockSignaturePubKeyKey]; exists {
		forbiddenPubKey = extraData[types.ForbiddenBlockSignaturePubKeyKey]

		if len(forbiddenPubKey) != btcec.PubKeyBytesLenCompressed {
			return 0, 0, nil, types.RuleErrorForbiddenPubKeyLength
		}
	...
	if totalOutput == 0 {
		return 0, 0, nil, types.RuleErrorUserOutputMustBeNonzero
	}
...
func (bav *UtxoView) _disconnectUpdateGlobalParams(
	...
	if prevGlobalParamEntry == nil {
		prevGlobalParamEntry = &types.InitialGlobalParamsEntry
	}
