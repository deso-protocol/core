...
func (bav *UtxoView) _connectSubmitPost(
	txn *network.MsgDeSoTxn, txHash *types.BlockHash, blockHeight uint32,
	...
	if !ignoreUtxos {
		...
		if totalInput == 0 {
			return 0, 0, nil, types.RuleErrorSubmitPostRequiresNonZeroInput
		}
	}
	...
	if quotedRepost, hasQuotedRepost := extraData[types.IsQuotedRepostKey]; hasQuotedRepost {
		if reflect.DeepEqual(quotedRepost, types.QuotedRepostVal) {
			isQuotedRepost = true
		}
		// Delete key since it is not needed in the PostExtraData map as IsQuotedRepost is involved in consensus code.
		delete(extraData, types.IsQuotedRepostKey)
	}
	var repostedPostHash *types.BlockHash
	if repostedPostHashBytes, isRepost := extraData[types.RepostedPostHash]; isRepost {
		repostedPostHash = &types.BlockHash{}
		copy(repostedPostHash[:], repostedPostHashBytes)
		delete(extraData, types.RepostedPostHash)
	}
	...
	if len(txMeta.PostHashToModify) != 0 {
		...
		if len(txMeta.PostHashToModify) != types.HashSizeBytes {
			return 0, 0, nil, errors.Wrapf(
				types.RuleErrorSubmitPostInvalidPostHashToModify,
		...
		if existingPostEntryy == nil || existingPostEntryy.isDeleted {
			return 0, 0, nil, errors.Wrapf(
				types.RuleErrorSubmitPostModifyingNonexistentPost,
		...
		if !reflect.DeepEqual(txn.PublicKey, existingPostEntryy.PosterPublicKey) {
			...
			return 0, 0, nil, errors.Wrapf(
				types.RuleErrorSubmitPostPostModificationNotAuthorized,
				"_connectSubmitPost: Post hash: %v, poster public key: %v, "+
					"txn public key: %v, paramUpdater: %v", postHash,
				types.PkToStringBoth(existingPostEntryy.PosterPublicKey),
		...
		if existingPostEntryy.IsNFT {
			return 0, 0, nil, errors.Wrapf(types.RuleErrorSubmitPostCannotUpdateNFT, "_connectSubmitPost: ")
		}
		...
		if !reflect.DeepEqual(repostedPostHash, existingPostEntryy.RepostedPostHash) {
			return 0, 0, nil, errors.Wrapf(
				types.RuleErrorSubmitPostUpdateRepostHash,
		...
		if isQuotedRepost != existingPostEntryy.IsQuotedRepost {
			return 0, 0, nil, errors.Wrapf(
				types.RuleErrorSubmitPostUpdateIsQuotedRepost,
		...
		*prevPostEntry = *existingPostEntryy
		...
		if txMeta.StakeMultipleBasisPoints < 100*100 ||
			...
			return 0, 0, nil, errors.Wrapf(types.RuleErrorSubmitPostStakeMultipleSize,
		...
		if txMeta.CreatorBasisPoints < 0 ||
			...
			return 0, 0, nil, errors.Wrapf(types.RuleErrorSubmitPostCreatorPercentageSize,
				"_connectSubmitPost: Invalid CreatorPercentageSize: %d",
				txMeta.CreatorBasisPoints)
		}
		// TstampNanos != 0
		if txMeta.TimestampNanos == 0 {
			return 0, 0, nil, errors.Wrapf(types.RuleErrorSubmitPostTimestampIsZero,
		...
		if len(txMeta.ParentStakeID) != 0 && len(txMeta.ParentStakeID) != types.HashSizeBytes &&
			len(txMeta.ParentStakeID) != btcec.PubKeyBytesLenCompressed {
			return 0, 0, nil, errors.Wrapf(types.RuleErrorSubmitPostInvalidParentStakeIDLength,
				"_connectSubmitPost: Parent stake ID length %v must be either 0 or %v or %v",
				len(txMeta.ParentStakeID), types.HashSizeBytes, btcec.PubKeyBytesLenCompressed)
		}
		...
		if existingPostEntry != nil && !existingPostEntry.isDeleted {
			return 0, 0, nil, errors.Wrapf(
				types.RuleErrorPostAlreadyExists,
				"_connectSubmitPost: Post hash: %v", postHash)
		}
		...
		if repostedPostHash != nil {
			...
			if newRepostedPostEntry == nil {
				return 0, 0, nil, types.RuleErrorSubmitPostRepostPostNotFound
			}
			// It is an error if a post is trying to repost a vanilla repost.
			if IsVanillaRepost(newRepostedPostEntry) {
				return 0, 0, nil, types.RuleErrorSubmitPostRepostOfRepost
			}
		}
...
func (bav *UtxoView) _getParentAndGrandparentPostEntry(postEntry *PostEntry) (
	...
	if parentPostEntry != nil && len(parentPostEntry.ParentStakeID) == types.HashSizeBytes {
...
func (bav *UtxoView) _disconnectSubmitPost(
	...
	txMeta := currentTxn.TxnMeta.(*network.SubmitPostMetadata)

	// The post hash is either the transaction hash or the hash set
	// in the metadata.
	postHashModified := txnHash
	if len(txMeta.PostHashToModify) != 0 {
		postHashModified = &types.BlockHash{}
...
func (bav *UtxoView) setPostMappings(post *types.PGPost) *PostEntry {
...
func (bav *UtxoView) GetParentPostEntriesForPostEntry(postEntry *PostEntry, maxDepth uint32, rootFirst bool,
	...
	if len(parentStakeID) != types.HashSizeBytes {
		return parentPostEntries, false
	}

	iterations := uint32(0)
	for len(parentStakeID) == types.HashSizeBytes && iterations < maxDepth {
		parentPostHash := &types.BlockHash{}
...
// applied to it.
func (bav *UtxoView) GetAllPosts() (_corePosts []*PostEntry, _commentsByPostHash map[types.BlockHash][]*PostEntry, _err error) {
	...
	commentsByPostHash := make(map[types.BlockHash][]*PostEntry)
	for _, postEntry := range bav.PostHashToPostEntry {
		// Ignore deleted or rolled-back posts.
		if postEntry.isDeleted {
			continue
...
func (bav *UtxoView) GetPostsPaginatedForPublicKeyOrderedByTimestamp(publicKey []byte, startPostHash *types.BlockHash, limit uint64, mediaRequired bool) (_posts []*PostEntry, _err error) {
	if bav.Postgres != nil {
...
func (bav *UtxoView) GetDiamondSendersForPostHash(postHash *types.BlockHash) (_pkidToDiamondLevel map[types.PKID]int64, _err error) {
	handle := bav.Handle
	...
	expectedKeyLength := 1 + types.HashSizeBytes + btcec.PubKeyBytesLenCompressed + 8
	for _, key := range keysFound {
		...
		senderPKID := &types.PKID{}
		copy(senderPKID[:], key[1+types.HashSizeBytes:])
	...
	pkidToDiamondLevel := make(map[types.PKID]int64)
...
func (bav *UtxoView) GetLikesForPostHash(postHash *types.BlockHash) (_likerPubKeys [][]byte, _err error) {
	if bav.Postgres != nil {
		...
		// Iterate over all the db keys & values and load them into the view.
		expectedKeyLength := 1 + types.HashSizeBytes + btcec.PubKeyBytesLenCompressed
		for _, key := range keysFound {
			// Sanity check that this is a reasonable key.
			if len(key) != expectedKeyLength {
			...
			likerPubKey := key[1+types.HashSizeBytes:]
...
func (bav *UtxoView) GetRepostsForPostHash(postHash *types.BlockHash) (_reposterPubKeys [][]byte, _err error) {
	handle := bav.Handle
	...
	for _, key := range keysFound {
		...
		reposterPubKey := key[1+types.HashSizeBytes:]
...
func (bav *UtxoView) GetQuoteRepostsForPostHash(postHash *types.BlockHash,
	...
	// Iterate over all the db keys & values and load them into the view.
	expectedKeyLength := 1 + types.HashSizeBytes + btcec.PubKeyBytesLenCompressed + types.HashSizeBytes

	repostPostHashIdx := 1 + types.HashSizeBytes + btcec.PubKeyBytesLenCompressed
	for _, key := range keysFound {
		// Sanity check that this is a reasonable key.
		if len(key) != expectedKeyLength {
		...
		repostPostHash := &types.BlockHash{}
...
func (bav *UtxoView) GetPostEntryReaderState(
	...
	if senderPKID == nil || receiverPKID == nil {
		glog.Debugf(
			...
			types.PkToString(readerPK, bav.Params), types.PkToString(postEntry.PosterPublicKey, bav.Params))
	} else {
...
func (bav *UtxoView) GetLikedByReader(readerPK []byte, postHash *types.BlockHash) bool {
	// Get like state.
...
func (bav *UtxoView) GetRepostPostEntryStateForReader(readerPK []byte, postHash *types.BlockHash) (string, bool) {
...
func (bav *UtxoView) GetPostEntryForPostHash(postHash *types.BlockHash) *PostEntry {
	// If an entry exists in the in-memory map, return the value of that mapping.
