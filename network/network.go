...
func WriteMessage(ww io.Writer, msg DeSoMessage, networkType types.NetworkType) ([]byte, error) {
	hdr := []byte{}
	...
	hash := types.Sha256DoubleHash(payload)
...
// derived.
func ReadMessage(rr io.Reader, networkType types.NetworkType) (DeSoMessage, []byte, error) {
	...
	if types.NetworkType(inNetworkType) != networkType {
		return nil, nil, fmt.Errorf("ReadMessage: Incorrect network type (%s) expected (%s)", types.NetworkType(inNetworkType), networkType)
	}
...
func NewMessage(msgType MsgType) DeSoMessage {
	...
	case MsgTypeBlock:
		{
			return &MsgDeSoBlock{
				Header: NewMessage(MsgTypeHeader).(*types.MsgDeSoHeader),
			}
		}
	case MsgTypeTxn:
		{
...
func (msg *MsgDeSoHeaderBundle) FromBytes(data []byte) error {
	...
	for ii := uint64(0); ii < numHeaders; ii++ {
		retHeader, err := types.DecodeHeader(rr)
		if err != nil {
	...
	retBundle.TipHash = &types.BlockHash{}
...
type InvVect struct {
	...
	Hash types.BlockHash // Hash of the data
}
...
func (bpi *BlockProducerInfo) String() string {
	...
	return fmt.Sprintf("Signer Key: %v", types.PkToStringMainnet(bpi.PublicKey))
}
...
func (msg *MsgDeSoBlock) ToBytes(preSignature bool) ([]byte, error) {
	if msg.Header.Version == types.HeaderVersion0 {
		return msg.EncodeBlockVersion0(preSignature)
	} else if msg.Header.Version == types.HeaderVersion1 {
...
func (msg *MsgDeSoBlock) FromBytes(data []byte) error {
	...
	// remains nil.
	if ret.Header.Version == types.HeaderVersion1 {
...
func (msg *MsgDeSoBlock) Hash() (*types.BlockHash, error) {
...
type DeSoInput types.UtxoKey

func (desoInput *DeSoInput) String() string {
	return (*types.UtxoKey)(desoInput).String()
}
...
func (desoOutput *DeSoOutput) String() string {
	return fmt.Sprintf("< PublicKey: %#v, AmountNanos: %d >",
		types.PkToStringMainnet(desoOutput.PublicKey), desoOutput.AmountNanos)
}
...
func (msg *MsgDeSoTxn) String() string {
	...
	if msg.TxnMeta.GetTxnType() == TxnTypeBitcoinExchange {
		pubKeyObj, err := ExtractBitcoinPublicKeyFromBitcoinTransactionInputs(
			msg.TxnMeta.(*BitcoinExchangeMetadata).BitcoinTransaction, types.DeSoMainnetParams.BitcoinBtcdParams)
		if err != nil {
			pubKey = msg.PublicKey
		} else {
	...
	return fmt.Sprintf("< TxHash: %v, TxnType: %v, PubKey: %v >",
		msg.Hash(), msg.TxnMeta.GetTxnType(), types.PkToStringMainnet(pubKey))
}
...
// transaction ID.
func (msg *MsgDeSoTxn) Hash() *types.BlockHash {
	...
	return types.Sha256DoubleHash(txBytes)
}
...
func (msg *MsgDeSoTxn) Sign(privKey *btcec.PrivateKey) (*btcec.Signature, error) {
	...
	txnSignatureHash := types.Sha256DoubleHash(txnBytes)
...
func SignTransactionWithDerivedKey(txnBytes []byte, privateKey *btcec.PrivateKey) ([]byte, []byte, error) {
	...
	txn.ExtraData[types.DerivedPublicKey] = privateKey.PubKey().SerializeCompressed()
...
type BitcoinExchangeMetadata struct {
	...
	BitcoinBlockHash *types.BlockHash
	...
	BitcoinMerkleRoot *types.BlockHash
...
func (txnData *BitcoinExchangeMetadata) FromBytes(data []byte) error {
	...
	// BitcoinBlockHash
	ret.BitcoinBlockHash = &types.BlockHash{}
	...
	// BitcoinMerkleRoot
	ret.BitcoinMerkleRoot = &types.BlockHash{}
...
func (txnData *LikeMetadata) ToBytes(preSignature bool) ([]byte, error) {
	...
	if len(txnData.LikedPostHash) != types.HashSizeBytes {
		return nil, fmt.Errorf("LikeMetadata.ToBytes: LikedPostHash "+
			"has length %d != %d", len(txnData.LikedPostHash), types.HashSizeBytes)
	}
...
func (txnData *LikeMetadata) FromBytes(data []byte) error {
	...
	// LikedPostHash
	ret.LikedPostHash = &types.BlockHash{}
...
func (txnData *CreateNFTMetadata) ToBytes(preSignature bool) ([]byte, error) {
	...
	//
	// Post hash must be included and must have the expected length.
	if len(txnData.NFTPostHash) != types.HashSizeBytes {
		return nil, fmt.Errorf("CreateNFTMetadata.ToBytes: NFTPostHash "+
			"has length %d != %d", len(txnData.NFTPostHash), types.HashSizeBytes)
	}
...
func (txnData *CreateNFTMetadata) FromBytes(dataa []byte) error {
	...
	// NFTPostHash
	ret.NFTPostHash = &types.BlockHash{}
...
func (txnData *AcceptNFTBidMetadata) FromBytes(dataa []byte) error {
	...
	ret.BidderPKID = types.PublicKeyToPKID(bidderPKIDBytes)
