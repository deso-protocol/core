...
func (fc *FastHotStuffConsensus) handleBlockProposalEvent(
	...
	missingBlockHashes, err := fc.tryProcessBlockAsNewTip(blockProposal, nil)
	if err != nil {
...
func (fc *FastHotStuffConsensus) tryProcessBlockAsNewTip(block *MsgDeSoBlock, hash *BlockHash) ([]*BlockHash, error) {
	if hash == nil {
		var err error
		hash, err = block.Hash()
		if err != nil {
			return nil, errors.Errorf("Error hashing block: %v", err)
		}
	}
