...
func (bav *UtxoView) _flushMessagingGroupEntriesMembersAttributesToDbWithTxn(txn *badger.Txn, blockHeight uint64) error {
	glog.V(2).Infof("_flushMessagingGroupEntriesMembersAttributesToDbWithTxn: flushing %d mappings", len(bav.MessagingGroupKeyToMessagingGroupEntry))
	numDeleted := 0
	numPut := 0
	...
	for messagingGroupKey, messagingGroupEntry := range bav.MessagingGroupKeyToMessagingGroupEntry {
		...
		if existingMessagingGroupEntry != nil {
			...
			if blockHeight >= uint64(bav.Params.ForkHeights.DeSoUnlimitedDerivedKeysAndMessagesMutingAndMembershipIndexBlockHeight) {
				members, err := DBGetAllGroupMembersForMessagingGroupWithTxn(txn, bav.Snapshot, messagingGroupEntry.GroupOwnerPublicKey, messagingGroupEntry.MessagingGroupKeyName)
				if err != nil {
					return errors.Wrapf(err, "UtxoView._flushMessagingGroupEntriesMembersAttributesToDbWithTxn: "+
				...
				for _, member := range members {
					...
					if err := DBDeleteMemberFromMembershipIndexWithTxn(txn, bav.Snapshot, member.GroupMemberPublicKey,
						...
						return errors.Wrapf(err, "UtxoView._flushMessagingGroupEntriesMembersAttributesToDbWithTxn: "+
							"Problem deleting MessagingGroupEntry recipients (%v) from db membership index", member)
					}
					...
					if err := DBDeleteMemberFromEnumerationIndexWithTxn(txn, bav.Snapshot,
						...
						return errors.Wrapf(err, "UtxoView._flushMessagingGroupEntriesMembersAttributesToDbWithTxn: "+
							"Problem deleting MessagingGroupEntry recipients (%v) from db enumeration index", member)
					}
				}
			} else {
		...
		if messagingGroupEntry.isDeleted {
			...
			if blockHeight >= uint64(bav.Params.ForkHeights.DeSoUnlimitedDerivedKeysAndMessagesMutingAndMembershipIndexBlockHeight) {
				...
				messagingGroupMembers, err := DBGetAllGroupMembersForMessagingGroupWithTxn(txn, bav.Snapshot, messagingGroupEntry.GroupOwnerPublicKey, messagingGroupEntry.MessagingGroupKeyName)
				if err != nil {
					return errors.Wrapf(err, "UtxoView._flushMessagingGroupEntriesMembersAttributesToDbWithTxn: "+
				...
				for _, member := range messagingGroupMembers {
					...
					if err != nil {
						return errors.Wrapf(err, "UtxoView._flushMessagingGroupEntriesMembersAttributesToDbWithTxn: "+
							"Fail while putting new enumeration index. Problem putting MessagingGroupEntry member (%v) to db", *member)
					}
				}
			} else {
	...
	for enumerationKey, mutedMembers := range bav.MutedMembers {
		...
		if err := DBDeleteMutedMembersForEnumerationKeyWithTxn(txn, bav.Snapshot, enumerationKey); err != nil {
			return errors.Wrapf(err, "UtxoView._flushMessagingGroupEntriesMembersAttributesToDbWithTxn: "+
				"Problem deleting MutedMembers for enumeration key (%v) from db", enumerationKey)
		}
