...
func (bav *UtxoView) GetSnapshotValidatorBLSPublicKeyPKIDPairEntry(blsPublicKey *bls.PublicKey, snapshotAtEpochNumber uint64) (*BLSPublicKeyPKIDPairEntry, error) {
	...
	// If we don't have it in the UtxoView, check the db.
	blsPublicKeyPKIDPairEntry, err := DBGetSnapshotValidatorBLSPublicKeyPKIDPairEntryByBLSPublicKey(bav.Handle, bav.Snapshot, blsPublicKey, snapshotAtEpochNumber)
	if err != nil {
...
func DBGetSnapshotValidatorBLSPublicKeyPKIDPairEntryByBLSPublicKey(handle *badger.DB, snap *Snapshot, blsPublicKey *bls.PublicKey, snapshotAtEpochNumber uint64) (*BLSPublicKeyPKIDPairEntry, error) {
	var ret *BLSPublicKeyPKIDPairEntry
	err := handle.View(func(txn *badger.Txn) error {
		var innerErr error
		ret, innerErr = DBGetSnapshotValidatorBLSPublicKeyPKIDPairEntryByBLSPublicKeyWithTxn(txn, snap, blsPublicKey, snapshotAtEpochNumber)
		return innerErr
	})
	return ret, err
}

func DBGetSnapshotValidatorBLSPublicKeyPKIDPairEntryByBLSPublicKeyWithTxn(txn *badger.Txn, snap *Snapshot, blsPublicKey *bls.PublicKey, snapshotAtEpochNumber uint64) (*BLSPublicKeyPKIDPairEntry, error) {
	// Retrieve from db.
	...
	if err != nil {
		...
		return nil, errors.Wrapf(err, "DBGetSnapshotValidatorBLSPublicKeyPKIDPairEntryByBLSPublicKeyWithTxn: problem retrieving BLSPublicKeyPKIDPairEntry")
	}
	...
	if exist, err := DecodeFromBytes(blsPublicKeyPKIDPairEntry, rr); !exist || err != nil {
		return nil, errors.Wrapf(err, "DBGetSnapshotValidatorBLSPublicKeyPKIDPairEntryByBLSPublicKeyWithTxn: problem decoding BLSPublicKeyPKIDPairEntry")
	}
	return blsPublicKeyPKIDPairEntry, nil
}
...
func (bav *UtxoView) _setSnapshotValidatorBLSPublicKeyPKIDPairEntry(blsPublicKeyPKIDPairEntry *BLSPublicKeyPKIDPairEntry, snapshotAtEpochNumber uint64) {
	if blsPublicKeyPKIDPairEntry == nil {
		glog.Errorf("_setValidatorBLSPublicKeyEntry: called with nil entry, this should never happen")
		return
	}
