...
// GetSnapshotValidatorSetsByBlockHeights returns the validator set for each block height provided.
// It requires all input block heights to be in the previous, current, or next epoch.
func (bav *UtxoView) GetSnapshotValidatorSetsByBlockHeights(blockHeights []uint64) (map[uint64][]*ValidatorEntry, error) {
