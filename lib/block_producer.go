...
func (desoBlockProducer *DeSoBlockProducer) _getBlockTemplate(publicKey []byte) (
	...
	if desoBlockProducer.chain.chainState() != SyncStateSyncingHeaders &&
		...
		for ii, mempoolTx := range txnsOrderedByTimeAdded {
			...
			_, _, _, _, err = utxoViewCopy._connectTransaction(mempoolTx.Tx, mempoolTx.Hash, int64(mempoolTx.TxSizeBytes), uint32(blockRet.Header.Height), true, false, false)
			if err != nil {
			...
			_, _, _, _, err = utxoView._connectTransaction(mempoolTx.Tx, mempoolTx.Hash, int64(mempoolTx.TxSizeBytes), uint32(blockRet.Header.Height), true, false, false)
			if err != nil {
	...
	for _, txnInBlock := range blockRet.Txns[1:] {
		var feeNanos uint64
		_, _, _, feeNanos, err = feesUtxoView._connectTransaction(txnInBlock, txnInBlock.Hash(), 0, uint32(blockRet.Header.Height), false, false, false)
		if err != nil {
			return nil, nil, nil, fmt.Errorf(
