...
func _getAuthorizeDerivedKeyMetadata(
	...
	accessSignature := ecdsa2.Sign(ownerPrivateKey, Sha256DoubleHash(accessBytes)[:])
...
func _getAuthorizeDerivedKeyMetadataWithTransactionSpendingLimit(
	...
	signature := ecdsa2.Sign(ownerPrivateKey, Sha256DoubleHash(accessBytes)[:])
...
func TestAuthorizeDerivedKeyBasic(t *testing.T) {
	...
	// an invalid private key. This must fail.
	{
		...
		accessSignatureRandom := ecdsa2.Sign(randomPrivateKey, Sha256DoubleHash(accessBytes)[:])
		require.NoError(err)
		_, _, _, err = _doAuthorizeTxn(
			testMeta,
			utxoView,
	...
	// Derived key for M1 mints 100 M1 DAO coins
	{
		...
		_doTxnWithTestMeta(
			...
			&DAOCoinMetadata{
				...
				CoinsToMintNanos: *uint256.NewInt(0).SetUint64(100 * NanosPerUnit),
	...
	// Derived key for M1 transfers 10 M1 DAO Coins to M0
	{
		...
		_doTxnWithTestMeta(
			...
			&DAOCoinTransferMetadata{
				...
				DAOCoinToTransferNanos: *uint256.NewInt(0).SetUint64(10 * NanosPerUnit),
			},
			nil,
			blockHeight+1,
		)
	...
	// Burning some DAO coins should work
	{
		...
		_doTxnWithTestMeta(
			...
			&DAOCoinMetadata{
				...
				CoinsToBurnNanos: *uint256.NewInt(0).SetUint64(10 * NanosPerUnit),
			},
			nil,
			blockHeight+1,
		)
