...
func (bav *UtxoView) getAdjustedDAOCoinBalanceForUserInBaseUnits(
	...
	if *daoCoinPKID == ZeroPKID {
		...
		return adjustBalance(
			uint256.NewInt(0).SetUint64(transactorDESOBalanceNanos), delta)
	}
...
func (bav *UtxoView) _connectDAOCoinLimitOrder(
	...
	if txMeta.CancelOrderID != nil {
		...
		if existingTransactorOrder == nil || (blockHeight >= bav.Params.ForkHeights.AssociationsAndAccessGroupsBlockHeight && existingTransactorOrder.isDeleted) {
...
func (bav *UtxoView) _disconnectDAOCoinLimitOrder(
	...
	numUtxoAdds := 0
	// We skip the first utxo op because it's the basic transfer to pay for the fee.
	for _, utxoOp := range utxoOpsForTxn[1:] {
