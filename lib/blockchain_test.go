...
func TestBasicTransferReorg(t *testing.T) {
	...
	for _, forkBlock := range forkBlocks {
		_, _, _, err := chain1.ProcessBlock(forkBlock, 0, true /*verifySignatures*/)
		require.NoError(err)
	}
...
func _shouldConnectBlock(blk *MsgDeSoBlock, t *testing.T, chain *Blockchain) {
	...
	isMainChain, isOrphan, _, err := chain.ProcessBlock(blk, 0, verifySignatures)
	require.NoError(err)
...
func TestProcessBlockReorgBlocks(t *testing.T) {
	...
	verifySignatures := true
	{
		...
		isMainChain, isOrphan, _, err := chain.ProcessBlock(blockB1, 0, verifySignatures)
		require.NoError(err)
...

	{
		...
		require.Equal(uint64(3), GetUtxoNumEntries(db, chain.snapshot))
		isMainChain, isOrphan, _, err := chain.ProcessBlock(blockB2, 0, verifySignatures)
		require.NoError(err)
...
func TestBadBlockSignature(t *testing.T) {
	...
	*blockProducerInfoCopy.Signature = *finalBlock1.BlockProducerInfo.Signature
	...
	_, _, _, err = chain.ProcessBlock(finalBlock1, 0, true)
	require.Error(err)
