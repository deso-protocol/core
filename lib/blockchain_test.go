...
func TestBasicTransferReorg(t *testing.T) {
	...
	for _, forkBlock := range forkBlocks {
		_, _, err := chain1.ProcessBlockPoW(forkBlock, true /*verifySignatures*/)
		require.NoError(err)
	}
...
func _shouldConnectBlock(blk *MsgDeSoBlock, t *testing.T, chain *Blockchain) {
	...
	isMainChain, isOrphan, err := chain.ProcessBlockPoW(blk, verifySignatures)
	require.NoError(err)
...
func TestProcessBlockReorgBlocks(t *testing.T) {
	...
	verifySignatures := true
	{
		...
		isMainChain, isOrphan, err := chain.ProcessBlockPoW(blockB1, verifySignatures)
		require.NoError(err)
...

	{
		...
		require.Equal(uint64(3), GetUtxoNumEntries(db, chain.snapshot))
		isMainChain, isOrphan, err := chain.ProcessBlockPoW(blockB2, verifySignatures)
		require.NoError(err)
...
func TestBadBlockSignature(t *testing.T) {
	...
	*blockProducerInfoCopy.Signature = *finalBlock1.BlockProducerInfo.Signature
	...
	_, _, err = chain.ProcessBlockPoW(finalBlock1, true)
	require.Error(err)
