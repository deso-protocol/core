...
func NewLowDifficultyBlockchainWithParamsAndDb(t *testing.T, params *DeSoParams, usePostgres bool, postgresPort uint32, useProvidedParams bool) (
	*Blockchain, *DeSoParams, *embeddedpostgres.EmbeddedPostgres) {
	...
	var snap *Snapshot
	if !usePostgres {
		snap, err, _, _ = NewSnapshot(db, SnapshotBlockHeightPeriod, false, false, &testParams, false, HypersyncDefaultMaxQueueSize, nil, nil)
		if err != nil {
			log.Fatal(err)
		}
	}
...
func TestProcessHeaderskReorgBlocks(t *testing.T) {
...

	{
		...
		require.NoError(err)
		_, isMainChain, isOrphan, err := chain.ProcessHeader(blockA1.Header, headerHash, false)
		require.NoError(err)
	...
	}
	{
		...
		require.NoError(err)
		_, isMainChain, isOrphan, err := chain.ProcessHeader(blockA2.Header, headerHash, false)
		require.NoError(err)
		require.True(isMainChain)
		require.False(isOrphan)
		// Make sure the tip lines up.
	...
	}
	{
		...
		require.NoError(err)
		_, isMainChain, isOrphan, err := chain.ProcessHeader(blockB1.Header, headerHash, false)
		require.NoError(err)
	...
	}
	{
		...
		require.NoError(err)
		_, isMainChain, isOrphan, err := chain.ProcessHeader(blockB2.Header, headerHash, false)
		require.NoError(err)
		// Should not be main chain yet
		require.False(isMainChain)
		require.False(isOrphan)
	...
	}
	{
		...
		require.NoError(err)
		_, isMainChain, isOrphan, err := chain.ProcessHeader(blockB3.Header, headerHash, false)
		require.NoError(err)
		// Should not be main chain yet
		require.True(isMainChain)
		require.False(isOrphan)
