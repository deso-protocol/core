...
func TestBalanceLedger(t *testing.T) {
	...
	require.NoError(balanceLedger.CanIncreaseBalance(pk0, 100, 100))
	require.NoError(balanceLedger.CanIncreaseBalance(pk0, 0, 100))
	balanceLedger.IncreaseBalance(pk0, 100)
	require.Equal(uint64(100), balanceLedger.GetBalance(pk0))
	require.NoError(balanceLedger.CanIncreaseBalance(pk0, 0, 100))
	require.Error(balanceLedger.CanIncreaseBalance(pk0, 1, 100))
	require.Error(balanceLedger.CanIncreaseBalance(pk0, 0, 99))
	require.Error(balanceLedger.CanIncreaseBalance(pk0, math.MaxUint64, math.MaxUint64))
	require.NoError(balanceLedger.CanDecreaseBalance(pk0, 100))
	require.NoError(balanceLedger.CanDecreaseBalance(pk0, 0))
	require.Error(balanceLedger.CanDecreaseBalance(pk0, 101))
	require.Error(balanceLedger.CanDecreaseBalance(pk0, math.MaxUint64))
