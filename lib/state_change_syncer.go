...
func (stateChangeSyncer *StateChangeSyncer) SyncMempoolToStateSyncer(server *Server) (bool, error) {
	...
	if err != nil {
		...
		fmt.Printf("After the mempool flush: %+v\n", &StateSyncerFlushedEvent{
			FlushId:        originalCommittedFlushId,
			Succeeded:      false,
			IsMempoolFlush: true,
		})
	...
	fmt.Printf("Time to connect all %d txns: %v\n", len(mempoolTxns), time.Since(startTime))
	startTime = time.Now()
