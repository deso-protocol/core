	...
	if uint32(len(msg.Headers)) < maxHeadersPerMsg || srv.blockchain.isTipMaxed(srv.blockchain.headerTip()) {
		...
		if srv.blockchain.chainState() == SyncStateSyncingBlocks {
			...
			if srv.snapshot != nil && srv.snapshot.CurrentEpochSnapshotMetadata != nil {
				srv.snapshot.CurrentEpochSnapshotMetadata.SnapshotBlockHeight = expectedSnapshotHeight
				srv.snapshot.CurrentEpochSnapshotMetadata.FirstSnapshotBlockHeight = expectedSnapshotHeight
				srv.snapshot.CurrentEpochSnapshotMetadata.CurrentEpochChecksumBytes = []byte{}
				srv.snapshot.CurrentEpochSnapshotMetadata.CurrentEpochBlockHash = srv.blockchain.bestHeaderChain[expectedSnapshotHeight].Hash
			}
