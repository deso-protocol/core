...
func (srv *Server) BroadcastTransaction(txn *MsgDeSoTxn) ([]*MsgDeSoTxn, error) {
	now := time.Now()
	glog.Infof("BroadcastTransaction: %v\n", now)
	txnHash := txn.Hash()
	if txnHash == nil {
		return nil, fmt.Errorf("BroadcastTransaction: Txn hash is nil")
	}
	glog.Infof("Adding transaction to mempool, elapsed: %v\n", time.Since(now))
	...
	glog.Infof("Transaction added to mempool, elapsed: %v\n", time.Since(now))
	...
	glog.Infof("Transaction validated, elapsed: %v\n", time.Since(now))
