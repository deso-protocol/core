...
type DBPrefixes struct {
	...
	//TODO (Michel) We need to include react emoji in these indices. I think the correct indices would be
	// 1. PrefixReactorPubKeyToPostHashReactEmojiL <prefix, user pub key, reacted post hash, react emoji>
	// 2. PrefixPostHashReactEmojiToReactorPubKey <prefix, reacted post hash, react emoji, user pub key>
	// this allows us to get all the different reactions a user gave to a single post easily (index 1) as well as get a list of all users who gave a "happy" react to a given post (index 2)
...
// -------------------------------------------------------------------------------------
//
//TODO (Michel) "reacted" instead of "liked" in all these DB functions
...
func (txnMeta *ReactTxindexMetadata) RawDecodeWithoutMetadata(blockHeight uint64, rr *bytes.Reader) error {
	...
	//TODO (Michel) I usually put isRemove as the last one. I think logically we would expect PostHashHex then EmojiReaction then IsRemove
