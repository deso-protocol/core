...
func (bav *UtxoView) DistributeStakingRewardsToSnapshotStakes(blockHeight uint64) error {
	...
	interestRateAsFloat := _convertScaledInterestRateToFloat(snapshotGlobalParamsEntry.StakingRewardInterestRatePerEpochScaled1e9)
...
const _stakingRewardInterestRateScalingFactor = uint64(1000000000) // 1e9

func _convertScaledInterestRateToFloat(scaledInterestRate uint64) *big.Float {
	scaledInterestRateFloat := NewFloat().SetUint64(scaledInterestRate)
	return scaledInterestRateFloat.Quo(scaledInterestRateFloat, NewFloat().SetUint64(_stakingRewardInterestRateScalingFactor))
}
