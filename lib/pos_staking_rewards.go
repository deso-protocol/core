...
func (bav *UtxoView) distributeStakingReward(
	...
	if stakeEntry != nil && stakeEntry.RewardMethod == StakingRewardMethodRestake {
		...
		stakeEntry.StakeAmountNanos = uint256.NewInt(0).Add(stakeEntry.StakeAmountNanos, uint256.NewInt(0).SetUint64(rewardNanos))
		bav._setStakeEntryMappings(stakeEntry)
		validatorEntry.TotalStakeAmountNanos = uint256.NewInt(0).Add(validatorEntry.TotalStakeAmountNanos, uint256.NewInt(0).SetUint64(rewardNanos))
