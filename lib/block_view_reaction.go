...
func (bav *UtxoView) _getReactionEntryForReactionKey(reactionKey *ReactionKey) *ReactionEntry {
	...
	if adapter.postgresDb != nil {
		reactionExists = adapter.postgresDb.GetReaction(reactionKey.ReactorPubKey[:], &reactionKey.ReactedPostHash, reactionKey.ReactEmoji) != nil
	} else {
		reactionExists = DbGetReactorPubKeyToPostHashMapping(adapter.badgerDb, reactionKey.ReactorPubKey[:], reactionKey.ReactedPostHash, reactionKey.ReactEmoji) != nil
	}
...
func (bav *UtxoView) GetReactorsForPostHash(postHash *BlockHash, reactionEmoji rune) (_ReactorPubKeys [][]byte, _err error) {
	// Returns the public key of the users that reacted to a post with a specific emoji reaction
	adapter := bav.GetDbAdapter()

	reactions, _ := adapter.GetReactionsForPost(postHash, reactionEmoji)

	for _, reaction := range reactions {
		reactKey := MakeReactionKey(reaction.ReactorPubKey, *reaction.ReactedPostHash, reactionEmoji)
		if _, exists := bav.ReactionKeyToReactionEntry[reactKey]; !exists {
			bav._setReactionEntryMappings(reaction)
		}
	}

	// Iterate over the view and create the final list to return
	// because there might be values in the view that weren't propagated to the db.
	var reactorPubKeys [][]byte
	for _, reactionEntry := range bav.ReactionKeyToReactionEntry {
		if !reactionEntry.isDeleted && bytes.Equal(reactionEntry.ReactedPostHash[:], postHash[:]) {
...
func (bav *UtxoView) _connectReact(
	...
	//TODO (Michel) Add a block height restriction before doing anything else in this function. You can see a fork height in constants.go
...
func (bav *UtxoView) _disconnectReact(
	...
	if txMeta.IsRemove {
		...
		// count. We do some sanity checks first though to be extra safe.
