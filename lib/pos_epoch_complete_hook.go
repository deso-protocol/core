...
func (bav *UtxoView) RunEpochCompleteHook(blockHeight uint64, viewNumber uint64, blockTimestampNanoSecs uint64) error {
	...
	// Step 3: Roll Over to The Next Epoch
	if err := bav.runEpochCompleteEpochRollover(currentEpochEntry.EpochNumber, blockHeight, viewNumber, blockTimestampNanoSecs); err != nil {
		return errors.Wrapf(err, "RunEpochCompleteHook: ")
	}
...
func (bav *UtxoView) runEpochCompleteEpochRollover(epochNumber uint64, blockHeight uint64, viewNumber uint64, blockTimestampNanoSecs uint64) error {
