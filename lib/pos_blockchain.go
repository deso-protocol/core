...
func (bvauo *BlockViewAndUtxoOps) Copy() (*BlockViewAndUtxoOps, error) {
	copiedView, err := bvauo.UtxoView.CopyUtxoView()
	if err != nil {
		return nil, errors.Wrapf(err, "BlockViewAndUtxoOps.Copy: Problem copying UtxoView")
	}
	return &BlockViewAndUtxoOps{
		UtxoView: copiedView,
		UtxoOps:  bvauo.UtxoOps,
		Block:    bvauo.Block,
	}, nil
}
...
// identifying all uncommitted ancestors of this block. Then it checks the block view cache
// to see if we have already computed this view. If not, connecting the uncommitted ancestor blocks
