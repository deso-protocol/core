...
func (bc *Blockchain) ProcessBlockPoS(block *MsgDeSoBlock, blockHash *BlockHash, currentView uint64, verifySignatures bool) (
	_success bool,
	_isOrphan bool,
...
// as validate failed.
func (bc *Blockchain) processOrphanBlockPoS(block *MsgDeSoBlock, hash *BlockHash) error {
	...
	if err = bc.isProperlyFormedBlockPoS(block); err != nil {
		if _, innerErr := bc.storeValidateFailedBlockInBlockIndex(block, hash); innerErr != nil {
	...
	_, err = bc.storeBlockInBlockIndex(block, hash)
...
func (bc *Blockchain) checkAndStoreArchivalBlock(block *MsgDeSoBlock, hash *BlockHash) (_success bool, _err error) {
	blockNode, exists := bc.blockIndexByHash.Get(*hash)
	...
	if _, err := bc.storeBlockInBlockIndex(block, hash); err != nil {
...
func (bc *Blockchain) validateLeaderAndQC(
	...
	if verifySignatures {
		...
		isValidPartialSig, err := parentUtxoView.hasValidProposerPartialSignaturePoS(
			block, blockHash, snapshotAtEpochNumber)
		if err != nil {
			return false, errors.Wrap(err,
...
func (bc *Blockchain) validateAndIndexBlockPoS(block *MsgDeSoBlock, blockHash *BlockHash, parentUtxoView *UtxoView, verifySignatures bool) (
	*BlockNode, error) {
	if blockHash == nil {
		return nil, fmt.Errorf("validateAndIndexBlockPoS: no block hash provided: %v", block)
	}
	...
	if parentBlockNode.IsValidateFailed() {
		return bc.storeValidateFailedBlockWithWrappedError(
			block, blockHash, errors.New("parent block is ValidateFailed"))
	}
	...
	if verifySignatures {
		...
		if !isValidRandomSeedSignature {
			return bc.storeValidateFailedBlockWithWrappedError(
				block, blockHash, errors.New("invalid random seed signature"))
		}
	}
...
func (bc *Blockchain) getOrCreateBlockNodeFromBlockIndex(block *MsgDeSoBlock, hash *BlockHash) (*BlockNode, error) {
	if hash == nil {
		return nil, fmt.Errorf("getOrCreateBlockNodeFromBlockIndex: Block hash is nil: %v", block)
	}
...
func (bc *Blockchain) storeValidateFailedHeaderInBlockIndexWithWrapperError(
...
func (bc *Blockchain) storeValidateFailedHeaderInBlockIndex(
	header *MsgDeSoHeader,
	hash *BlockHash,
) (*BlockNode, error) {
	blockNode, err := bc.getOrCreateBlockNodeFromBlockIndex(&MsgDeSoBlock{Header: header}, hash)
...
func (bc *Blockchain) upsertBlockAndBlockNodeToDB(block *MsgDeSoBlock, blockNode *BlockNode, storeFullBlock bool,
	...
	err := bc.db.Update(func(txn *badger.Txn) error {
		if storeFullBlock {
			// TODO: we can make PutBlockHashToBlockWithTxn faster by passing in the hash instead of
			// computing it from the block.
