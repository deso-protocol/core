...
func _testStaking(t *testing.T, flushToDB bool) {
	...
	}
	{
		...
		stakeMetadata := &StakeMetadata{
			...
			RewardMethod:       StakingRewardMethodPayToBalance,
			StakeAmountNanos:   uint256.NewInt(0),
		}
		_, err = _submitStakeTxn(
			testMeta, m1Pub, m1Priv, stakeMetadata, nil, flushToDB,
		)
	...
	}
	{
		...
		stakeMetadata := &StakeMetadata{
			ValidatorPublicKey: NewPublicKey(m0PkBytes),
			RewardMethod:       99,
			StakeAmountNanos:   uint256.NewInt(0).SetUint64(1),
		}
		_, err = _submitStakeTxn(
			testMeta, m1Pub, m1Priv, stakeMetadata, nil, flushToDB,
		)
	...
	}
	{
		...
		stakeMetadata := &StakeMetadata{
			ValidatorPublicKey: NewPublicKey(m0PkBytes),
			RewardMethod:       StakingRewardMethodPayToBalance,
			StakeAmountNanos:   uint256.NewInt(0).SetUint64(math.MaxUint64),
		}
		_, err = _submitStakeTxn(
			testMeta, m1Pub, m1Priv, stakeMetadata, nil, flushToDB,
		)
	...
	}
	{
		...
		require.Equal(t, stakeEntry.StakeAmountNanos, uint256.NewInt(0).SetUint64(100))
	...
	}
	{
		...
		stakeMetadata := &StakeMetadata{
			ValidatorPublicKey: NewPublicKey(m0PkBytes),
			RewardMethod:       StakingRewardMethodPayToBalance,
			StakeAmountNanos:   uint256.NewInt(0).SetUint64(50),
		}
		...
		require.NotNil(t, stakeEntry)
		require.Equal(t, stakeEntry.RewardMethod, StakingRewardMethodPayToBalance)
		require.Equal(t, stakeEntry.StakeAmountNanos, uint256.NewInt(0).SetUint64(150))
		...
		require.NoError(t, err)
		require.NotNil(t, validatorEntry)
		require.Equal(t, validatorEntry.TotalStakeAmountNanos, uint256.NewInt(0).SetUint64(150))
	...
	//
	{
		...
		unstakeMetadata := &UnstakeMetadata{
			ValidatorPublicKey: NewPublicKey(m0PkBytes),
			UnstakeAmountNanos: uint256.NewInt(0).SetUint64(40),
		}
	...
	}
	{
		...
		unstakeMetadata := &UnstakeMetadata{
			ValidatorPublicKey: NewPublicKey(m0PkBytes),
			UnstakeAmountNanos: uint256.NewInt(0),
		}
		_, err = _submitUnstakeTxn(
			testMeta, m1Pub, m1Priv, unstakeMetadata, nil, flushToDB,
		)
	...
	}
	{
		...
		stakeEntry, err := utxoView().GetStakeEntry(m0PKID, m1PKID)
		require.NoError(t, err)
		require.Equal(t, stakeEntry.StakeAmountNanos, uint256.NewInt(0).SetUint64(110))

		// Verify ValidatorEntry.TotalStakeAmountNanos.
		validatorEntry, err := utxoView().GetValidatorByPKID(m0PKID)
		require.NoError(t, err)
		require.Equal(t, validatorEntry.TotalStakeAmountNanos, uint256.NewInt(0).SetUint64(110))
		...
		require.NoError(t, err)
		require.Equal(t, lockedStakeEntry.LockedAmountNanos, uint256.NewInt(0).SetUint64(40))
	...
	}
	{
		...
		unstakeMetadata := &UnstakeMetadata{
			ValidatorPublicKey: NewPublicKey(m0PkBytes),
			UnstakeAmountNanos: uint256.NewInt(0).SetUint64(30),
		}
		extraData := map[string][]byte{"TestKey": []byte("TestValue2")}
		feeNanos, err := _submitUnstakeTxn(
			testMeta, m1Pub, m1Priv, unstakeMetadata, extraData, flushToDB,
		...
		stakeEntry, err := utxoView().GetStakeEntry(m0PKID, m1PKID)
		require.NoError(t, err)
		require.Equal(t, stakeEntry.StakeAmountNanos, uint256.NewInt(0).SetUint64(80))

		// Verify ValidatorEntry.TotalStakeAmountNanos.
		validatorEntry, err := utxoView().GetValidatorByPKID(m0PKID)
		require.NoError(t, err)
		require.Equal(t, validatorEntry.TotalStakeAmountNanos, uint256.NewInt(0).SetUint64(80))

		// Verify LockedStakeEntry.UnstakeAmountNanos.
		lockedStakeEntry, err := utxoView().GetLockedStakeEntry(m0PKID, m1PKID, currentEpochNumber)
		require.NoError(t, err)
		require.Equal(t, lockedStakeEntry.LockedAmountNanos, uint256.NewInt(0).SetUint64(70))
	...
	}
	{
		...
		unstakeMetadata := &UnstakeMetadata{
			ValidatorPublicKey: NewPublicKey(m0PkBytes),
			UnstakeAmountNanos: uint256.NewInt(0).SetUint64(80),
		}
		feeNanos, err := _submitUnstakeTxn(
			testMeta, m1Pub, m1Priv, unstakeMetadata, nil, flushToDB,
		)
		...
		validatorEntry, err := utxoView().GetValidatorByPKID(m0PKID)
		require.NoError(t, err)
		require.Equal(t, validatorEntry.TotalStakeAmountNanos, uint256.NewInt(0))

		// Verify LockedStakeEntry.UnstakeAmountNanos.
		lockedStakeEntry, err := utxoView().GetLockedStakeEntry(m0PKID, m1PKID, currentEpochNumber)
		require.NoError(t, err)
		require.Equal(t, lockedStakeEntry.LockedAmountNanos, uint256.NewInt(0).SetUint64(150))
		require.Equal(t, lockedStakeEntry.ExtraData["TestKey"], []byte("TestValue2"))
	...
	}
	{
		...
		require.Equal(t, lockedStakeEntries[0].LockedAmountNanos, uint256.NewInt(0).SetUint64(150))
...
func TestStakingWithDerivedKey(t *testing.T) {
	...
	}
	{
		...
		txnSpendingLimit := &TransactionSpendingLimit{
			...
			StakeLimitMap: map[StakeLimitKey]*uint256.Int{stakeLimitKey: uint256.NewInt(0).SetUint64(100)},
		}
	...
	}
	{
		...
		txnSpendingLimit := &TransactionSpendingLimit{
			...
			UnstakeLimitMap: map[StakeLimitKey]*uint256.Int{stakeLimitKey: uint256.NewInt(0).SetUint64(50)},
		}
		derivedKeyPriv, err = _submitAuthorizeDerivedKeyTxn(txnSpendingLimit)
		require.NoError(t, err)
		...
		unstakeMetadata = &UnstakeMetadata{
			ValidatorPublicKey: NewPublicKey(m0PkBytes),
			UnstakeAmountNanos: uint256.NewInt(0).SetUint64(200),
		}
		_, err = _submitStakeTxnWithDerivedKey(
			senderPkBytes, derivedKeyPriv, MsgDeSoTxn{TxnMeta: unstakeMetadata},
		)
		...
		require.NoError(t, err)
		require.NotNil(t, stakeEntry)
		require.Equal(t, stakeEntry.StakeAmountNanos, uint256.NewInt(0).SetUint64(50))
		...
		require.Equal(t, lockedStakeEntry.LockedAmountNanos, uint256.NewInt(0).SetUint64(50))
	}
	{
	...
	}
	{
		...
		txnSpendingLimit := &TransactionSpendingLimit{
			...
			StakeLimitMap:       map[StakeLimitKey]*uint256.Int{stakeLimitKey: uint256.NewInt(0).SetUint64(50)},
			UnstakeLimitMap:     map[StakeLimitKey]*uint256.Int{stakeLimitKey: uint256.NewInt(0).SetUint64(50)},
		...
		stakeMetadata := &StakeMetadata{
			ValidatorPublicKey: NewPublicKey(m0PkBytes),
			StakeAmountNanos:   uint256.NewInt(0).SetUint64(25),
		}
		_, err = _submitStakeTxn(
			testMeta, senderPkString, senderPrivString, stakeMetadata, nil, true,
		)
		...
		unstakeMetadata := &UnstakeMetadata{
			ValidatorPublicKey: NewPublicKey(m0PkBytes),
			UnstakeAmountNanos: uint256.NewInt(0).SetUint64(25),
		}
		_, err = _submitUnstakeTxn(
			testMeta, senderPkString, senderPrivString, unstakeMetadata, nil, true,
		)
	...
	}
	{
		...
		txnSpendingLimit := &TransactionSpendingLimit{
			...
			StakeLimitMap: map[StakeLimitKey]*uint256.Int{
				scopedStakeLimitKey: uint256.NewInt(0).SetUint64(100),
				globalStakeLimitKey: uint256.NewInt(0).SetUint64(200),
			},
	...
	}
	{
		...
		txnSpendingLimit := &TransactionSpendingLimit{
			...
			StakeLimitMap: map[StakeLimitKey]*uint256.Int{
				stakeLimitKey1: uint256.NewInt(0).SetUint64(uint64(1.5 * float64(NanosPerUnit))),
				stakeLimitKey2: uint256.NewInt(0).SetUint64(uint64(2.0 * float64(NanosPerUnit))),
			},
			UnstakeLimitMap: map[StakeLimitKey]*uint256.Int{
				stakeLimitKey1: uint256.NewInt(0).SetUint64(uint64(3.25 * float64(NanosPerUnit))),
			},
	...
	}
	{
		...
		txnSpendingLimit := &TransactionSpendingLimit{
			...
			StakeLimitMap: map[StakeLimitKey]*uint256.Int{
				stakeLimitKey: uint256.NewInt(0).SetUint64(uint64(0.65 * float64(NanosPerUnit))),
			},
			UnstakeLimitMap: map[StakeLimitKey]*uint256.Int{
				stakeLimitKey: uint256.NewInt(0).SetUint64(uint64(2.1 * float64(NanosPerUnit))),
			},
			UnlockStakeLimitMap: map[StakeLimitKey]uint64{stakeLimitKey: 1},
		}
		metamaskStr := txnSpendingLimit.ToMetamaskString(params)
...
func _testGetTopStakesByStakeAmount(t *testing.T, flushToDB bool) {
	...
	constructAndSubmitStakeTxn := func(stakerPk string, stakerPriv string, validatorPkBytes []byte, amountNanos uint64) {
		stakeMetadata := &StakeMetadata{
			...
			RewardMethod:       StakingRewardMethodPayToBalance,
			StakeAmountNanos:   uint256.NewInt(0).SetUint64(amountNanos),
		}
