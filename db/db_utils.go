...
func DBGetPKIDEntryForPublicKeyWithTxn(txn *badger.Txn, publicKey []byte) *block_view.PKIDEntry {
	if len(publicKey) == 0 {
		return nil
	}
	...
	// So return that pkid.
	pkidEntryObj := &block_view.PKIDEntry{}
	...
	if err != nil {
		glog.Errorf("DBGetPKIDEntryForPublicKeyWithTxn: Problem reading "+
			...
			types.PkToStringMainnet(publicKey))
		return nil
	}
	return pkidEntryObj
}

func DBGetPKIDEntryForPublicKey(db *badger.DB, publicKey []byte) *block_view.PKIDEntry {
	var pkid *block_view.PKIDEntry
...
func DBGetPublicKeyForPKIDWithTxn(txn *badger.Txn, pkidd *types.PKID) []byte {
...
func DBGetPublicKeyForPKID(db *badger.DB, pkidd *types.PKID) []byte {
	var publicKey []byte
	db.View(func(txn *badger.Txn) error {
...
func DBPutPKIDMappingsWithTxn(
	txn *badger.Txn, publicKey []byte, pkidEntry *block_view.PKIDEntry, params *types.DeSoParams) error {
	...
	// Set the main pub key -> pkid mapping.
	{
		...
		if err := txn.Set(pubKeyToPkidKey, pkidDataBuf.Bytes()); err != nil {
			...
			return errors.Wrapf(err, "DBPutPKIDMappingsWithTxn: Problem "+
				...
				types.PkToString(pkidEntry.PKID[:], params), types.PkToString(publicKey, params))
		}
	}
...
func DBDeletePKIDMappingsWithTxn(
	txn *badger.Txn, publicKey []byte, params *types.DeSoParams) error {
...

	{
		...
		if err := txn.Delete(pubKeyToPkidKey); err != nil {
			...
			return errors.Wrapf(err, "DBDeletePKIDMappingsWithTxn: Problem "+
				...
				types.PkToString(publicKey, params))
		}
	}

	{
		...
		if err := txn.Delete(pubKeyToPkidKey); err != nil {
			...
			return errors.Wrapf(err, "DBDeletePKIDMappingsWithTxn: Problem "+
				...
				types.PkToString(pkidEntry.PKID[:], params))
		}
	}
...
func DbGetDeSoBalanceNanosForPublicKeyWithTxn(txn *badger.Txn, publicKey []byte,
	...
	if err != nil {
		return uint64(0), errors.Wrapf(
			...
			types.PkToStringBoth(publicKey))
	}
...
func DbPutDeSoBalanceForPublicKeyWithTxn(
	...
	if err := txn.Set(_dbKeyForPublicKeyToDeSoBalanceNanos(publicKey), balanceBytes); err != nil {
		...
		return errors.Wrapf(
			...
			balanceNanos, types.PkToStringBoth(publicKey))
	}
...
func DbDeletePublicKeyToDeSoBalanceWithTxn(txn *badger.Txn, publicKey []byte) error {
	...
	if err := txn.Delete(_dbKeyForPublicKeyToDeSoBalanceNanos(publicKey)); err != nil {
		return errors.Wrapf(err, "DbDeletePublicKeyToDeSoBalanceWithTxn: Problem deleting "+
			"balance for public key %s", types.PkToStringMainnet(publicKey))
	}
...
func DbPutMessageEntryWithTxn(
	txn *badger.Txn, messageEntry *block_view.MessageEntry) error {
	...
	messageData := &block_view.MessageEntry{
...
func DbPutMessageEntry(handle *badger.DB, messageEntry *block_view.MessageEntry) error {
...
func DbGetMessageEntryWithTxn(
	txn *badger.Txn, publicKey []byte, tstampNanos uint64) *block_view.MessageEntry {

	key := _dbKeyForMessageEntry(publicKey, tstampNanos)
	privateMessageObj := &block_view.MessageEntry{}
	...
	if err != nil {
		glog.Errorf("DbGetMessageEntryWithTxn: Problem reading "+
			...
			types.PkToStringMainnet(publicKey), tstampNanos)
		return nil
	}
	return privateMessageObj
}

func DbGetMessageEntry(db *badger.DB, publicKey []byte, tstampNanos uint64) *block_view.MessageEntry {
	var ret *block_view.MessageEntry
	db.View(func(txn *badger.Txn) error {
...
func DbDeleteMessageEntryMappingsWithTxn(
	...
	if err := txn.Delete(_dbKeyForMessageEntry(existingMessage.SenderPublicKey, tstampNanos)); err != nil {
		return errors.Wrapf(err, "DbDeleteMessageEntryMappingsWithTxn: Deleting "+
			...
			types.PkToStringMainnet(existingMessage.SenderPublicKey), tstampNanos)
	}
	if err := txn.Delete(_dbKeyForMessageEntry(existingMessage.RecipientPublicKey, tstampNanos)); err != nil {
		return errors.Wrapf(err, "DbDeleteMessageEntryMappingsWithTxn: Deleting "+
			...
			types.PkToStringMainnet(existingMessage.RecipientPublicKey), tstampNanos)
	}
...
func DbGetMessageEntriesForPublicKey(handle *badger.DB, publicKey []byte) (
	_privateMessages []*block_view.MessageEntry, _err error) {
	...
	privateMessages := []*block_view.MessageEntry{}
...
func DbGetLimitedMessageEntriesForPublicKey(handle *badger.DB, publicKey []byte) (
	...
	// Goes backwards to get messages in time sorted order.
	// Limit the number of keys to speed up load times.
	_, valuesFound := _enumerateLimitedKeysReversedForPrefix(handle, prefix, uint64(types.MessagesToFetchPerInboxCall))
...
func DbDeleteForbiddenBlockSignaturePubKeyWithTxn(txn *badger.Txn, publicKey []byte) error {
	...
	if err := txn.Delete(_dbKeyForForbiddenBlockSignaturePubKeys(publicKey)); err != nil {
		return errors.Wrapf(err, "DbDeleteForbiddenBlockSignaturePubKeyWithTxn: Deleting "+
			"sender mapping for public key %s failed", types.PkToStringMainnet(publicKey))
	}
...
func _dbKeyForLikerPubKeyToLikedPostHashMapping(
	userPubKey []byte, likedPostHash types.BlockHash) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbKeyForLikedPostHashToLikerPubKeyMapping(
	likedPostHash types.BlockHash, userPubKey []byte) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbSeekPrefixForLikerPubKeysLikingAPostHash(likedPostHash types.BlockHash) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
	prefixCopy := append([]byte{}, _PrefixLikedPostHashToLikerPubKey...)
...
func DbPutLikeMappingsWithTxn(
	txn *badger.Txn, userPubKey []byte, likedPostHash types.BlockHash) error {
...
func DbPutLikeMappings(
	handle *badger.DB, userPubKey []byte, likedPostHash types.BlockHash) error {
...
func DbGetLikerPubKeyToLikedPostHashMappingWithTxn(
	txn *badger.Txn, userPubKey []byte, likedPostHash types.BlockHash) []byte {
...
func DbGetLikerPubKeyToLikedPostHashMapping(
	db *badger.DB, userPubKey []byte, likedPostHash types.BlockHash) []byte {
	var ret []byte
	db.View(func(txn *badger.Txn) error {
...
func DbGetLikerPubKeysLikingAPostHash(handle *badger.DB, likedPostHash types.BlockHash) (
	...
	for _, keyBytes := range keysFound {
		...
		userPubKey := keyBytes[1+types.HashSizeBytes:]
...
func _dbKeyForReposterPubKeyRepostedPostHashToRepostPostHash(userPubKey []byte, repostedPostHash types.BlockHash) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbKeyForRepostedPostHashReposterPubKey(repostedPostHash *types.BlockHash, reposterPubKey []byte) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbKeyForRepostedPostHashReposterPubKeyRepostPostHash(
	repostedPostHash *types.BlockHash, reposterPubKey []byte, repostPostHash *types.BlockHash) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func DbPutRepostMappingsWithTxn(
	txn *badger.Txn, userPubKey []byte, repostedPostHash types.BlockHash, repostEntry block_view.RepostEntry) error {
...
func DbPutRepostMappings(
	handle *badger.DB, userPubKey []byte, repostedPostHash types.BlockHash, repostEntry block_view.RepostEntry) error {
...
func DbGetReposterPubKeyRepostedPostHashToRepostEntryWithTxn(
	txn *badger.Txn, userPubKey []byte, repostedPostHash types.BlockHash) *block_view.RepostEntry {

	key := _dbKeyForReposterPubKeyRepostedPostHashToRepostPostHash(userPubKey, repostedPostHash)
	repostEntryObj := &block_view.RepostEntry{}
...
func DbReposterPubKeyRepostedPostHashToRepostEntry(
	db *badger.DB, userPubKey []byte, repostedPostHash types.BlockHash) *block_view.RepostEntry {
	var ret *block_view.RepostEntry
	db.View(func(txn *badger.Txn) error {
...
func DbDeleteRepostMappingsWithTxn(
	txn *badger.Txn, userPubKey []byte, repostedPostHash types.BlockHash) error {
	...
	if err := txn.Delete(_dbKeyForReposterPubKeyRepostedPostHashToRepostPostHash(userPubKey, repostedPostHash)); err != nil {
		return errors.Wrapf(err, "DbDeleteRepostMappingsWithTxn: Deleting "+
			...
			types.PkToStringMainnet(userPubKey[:]), types.PkToStringMainnet(repostedPostHash[:]))
	}
	return nil
}

func DbDeleteRepostMappings(
	handle *badger.DB, userPubKey []byte, repostedPostHash types.BlockHash) error {
	return handle.Update(func(txn *badger.Txn) error {
...
func DbGetPostHashesYouRepost(handle *badger.DB, yourPublicKey []byte) (
	...
	postHashesYouRepost := []*types.BlockHash{}
	for _, keyBytes := range keysFound {
...
func _dbKeyForFollowerToFollowedMapping(
	followerPKID *types.PKID, followedPKID *types.PKID) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbKeyForFollowedToFollowerMapping(
	followedPKID *types.PKID, followerPKID *types.PKID) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbSeekPrefixForPKIDsYouFollow(yourPKID *types.PKID) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
	prefixCopy := append([]byte{}, _PrefixFollowerPKIDToFollowedPKID...)
	return append(prefixCopy, yourPKID[:]...)
}

func _dbSeekPrefixForPKIDsFollowingYou(yourPKID *types.PKID) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
	prefixCopy := append([]byte{}, _PrefixFollowedPKIDToFollowerPKID...)
	return append(prefixCopy, yourPKID[:]...)
}
...
func DbPutFollowMappingsWithTxn(
	txn *badger.Txn, followerPKID *types.PKID, followedPKID *types.PKID) error {
...
func DbPutFollowMappings(
	handle *badger.DB, followerPKID *types.PKID, followedPKID *types.PKID) error {
...
func DbGetFollowerToFollowedMappingWithTxn(
	txn *badger.Txn, followerPKID *types.PKID, followedPKID *types.PKID) []byte {
...
func DbGetFollowerToFollowedMapping(db *badger.DB, followerPKID *types.PKID, followedPKID *types.PKID) []byte {
	var ret []byte
	db.View(func(txn *badger.Txn) error {
...
func DbDeleteFollowMappingsWithTxn(
	...
	if err := txn.Delete(_dbKeyForFollowerToFollowedMapping(followerPKID, followedPKID)); err != nil {
		return errors.Wrapf(err, "DbDeleteFollowMappingsWithTxn: Deleting "+
			...
			types.PkToStringMainnet(followerPKID[:]), types.PkToStringMainnet(followedPKID[:]))
	}
	if err := txn.Delete(_dbKeyForFollowedToFollowerMapping(followedPKID, followerPKID)); err != nil {
		return errors.Wrapf(err, "DbDeleteFollowMappingsWithTxn: Deleting "+
			...
			types.PkToStringMainnet(followedPKID[:]), types.PkToStringMainnet(followerPKID[:]))
	}
...
func DbGetPKIDsYouFollow(handle *badger.DB, yourPKID *types.PKID) (
	_pkids []*types.PKID, _err error) {

	prefix := _dbSeekPrefixForPKIDsYouFollow(yourPKID)
	keysFound, _ := _enumerateKeysForPrefix(handle, prefix)

	pkidsYouFollow := []*types.PKID{}
	for _, keyBytes := range keysFound {
...
func DbGetPKIDsFollowingYou(handle *badger.DB, yourPKID *types.PKID) (
	_pkids []*types.PKID, _err error) {

	prefix := _dbSeekPrefixForPKIDsFollowingYou(yourPKID)
	keysFound, _ := _enumerateKeysForPrefix(handle, prefix)

	pkidsFollowingYou := []*types.PKID{}
	for _, keyBytes := range keysFound {
...
func _dbKeyForDiamondReceiverToDiamondSenderMapping(diamondEntry *block_view.DiamondEntry) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbKeyForDiamondReceiverToDiamondSenderMappingWithoutEntry(
	diamondReceiverPKID *types.PKID, diamondSenderPKID *types.PKID, diamondPostHash *types.BlockHash) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
	prefixCopy := append([]byte{}, _PrefixDiamondReceiverPKIDDiamondSenderPKIDPostHash...)
...
func _dbKeyForDiamondedPostHashDiamonderPKIDDiamondLevel(diamondEntry *block_view.DiamondEntry) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbSeekPrefixForPKIDsThatDiamondedYou(yourPKID *types.PKID) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
	prefixCopy := append([]byte{}, _PrefixDiamondReceiverPKIDDiamondSenderPKIDPostHash...)
	return append(prefixCopy, yourPKID[:]...)
}

func _dbKeyForDiamondSenderToDiamondReceiverMapping(diamondEntry *block_view.DiamondEntry) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbSeekPrefixForPKIDsThatYouDiamonded(yourPKID *types.PKID) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
	prefixCopy := append([]byte{}, _PrefixDiamondSenderPKIDDiamondReceiverPKIDPostHash...)
	return append(prefixCopy, yourPKID[:]...)
}

func _dbSeekPrefixForReceiverPKIDAndSenderPKID(receiverPKID *types.PKID, senderPKID *types.PKID) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
	prefixCopy := append([]byte{}, _PrefixDiamondReceiverPKIDDiamondSenderPKIDPostHash...)
...
func _DbBufForDiamondEntry(diamondEntry *block_view.DiamondEntry) []byte {
	diamondEntryBuf := bytes.NewBuffer([]byte{})
	gob.NewEncoder(diamondEntryBuf).Encode(diamondEntry)
	return diamondEntryBuf.Bytes()
}

func _DbDiamondEntryForDbBuf(buf []byte) *block_view.DiamondEntry {
	if len(buf) == 0 {
		return nil
	}
	ret := &block_view.DiamondEntry{}
...
func DbPutDiamondMappingsWithTxn(
	txn *badger.Txn,
	diamondEntry *block_view.DiamondEntry) error {
...
func DbGetDiamondMappingsWithTxn(
	txn *badger.Txn, diamondReceiverPKID *types.PKID, diamondSenderPKID *types.PKID, diamondPostHash *types.BlockHash) *block_view.DiamondEntry {
...
func DbGetDiamondMappings(
	db *badger.DB, diamondReceiverPKID *types.PKID, diamondSenderPKID *types.PKID, diamondPostHash *types.BlockHash) *block_view.DiamondEntry {
	var ret *block_view.DiamondEntry
	db.View(func(txn *badger.Txn) error {
...
func DbDeleteDiamondMappingsWithTxn(txn *badger.Txn, diamondEntry *block_view.DiamondEntry) error {
	...
	if err := txn.Delete(_dbKeyForDiamondReceiverToDiamondSenderMapping(diamondEntry)); err != nil {
		return errors.Wrapf(err, "DbDeleteDiamondMappingsWithTxn: Deleting "+
			...
			types.PkToStringMainnet(diamondEntry.ReceiverPKID[:]),
			types.PkToStringMainnet(diamondEntry.SenderPKID[:]),
...
func DbDeleteDiamondMappings(handle *badger.DB, diamondEntry *block_view.DiamondEntry) error {
	return handle.Update(func(txn *badger.Txn) error {
...
func DbGetPKIDsThatDiamondedYouMap(handle *badger.DB, yourPKID *types.PKID, fetchYouDiamonded bool) (
	_pkidToDiamondsMap map[types.PKID][]*block_view.DiamondEntry, _err error) {
	...
	pkidsToDiamondEntryMap := make(map[types.PKID][]*block_view.DiamondEntry)
	for ii, keyBytes := range keysFound {
		...
		if diamondEntry == nil {
			return nil, fmt.Errorf(
				...
				types.PkToStringMainnet(yourPKID[:]), keyBytes)
		}
		expectedDiamondKeyLen := 1 + 2*btcec.PubKeyBytesLenCompressed + types.HashSizeBytes
		...
		if !reflect.DeepEqual(diamondSenderPKID, diamondEntry.SenderPKID) {
			return nil, fmt.Errorf(
				...
				types.PkToStringBoth(diamondSenderPKID[:]), types.PkToStringBoth(diamondEntry.SenderPKID[:]))
		}

		// Chop out the diamond receiver PKID
		diamondReceiverPKIDBytes := keyBytes[diamondReceiverStartIdx:diamondReceiverEndIdx]
		diamondReceiverPKID := &types.PKID{}
		...
		if !reflect.DeepEqual(diamondReceiverPKID, diamondEntry.ReceiverPKID) {
			return nil, fmt.Errorf(
				...
				types.PkToStringBoth(diamondReceiverPKID[:]), types.PkToStringBoth(diamondEntry.ReceiverPKID[:]))
		}
...
func DbGetDiamondEntriesForSenderToReceiver(handle *badger.DB, receiverPKID *types.PKID, senderPKID *types.PKID) (
	_diamondEntries []*block_view.DiamondEntry, _err error) {

	prefix := _dbSeekPrefixForReceiverPKIDAndSenderPKID(receiverPKID, senderPKID)
	keysFound, valsFound := _enumerateKeysForPrefix(handle, prefix)
	var diamondEntries []*block_view.DiamondEntry
	for ii, keyBytes := range keysFound {
		// The DiamondEntry found must not be nil.
		diamondEntry := _DbDiamondEntryForDbBuf(valsFound[ii])
		if diamondEntry == nil {
			return nil, fmt.Errorf(
				...
				types.PkToStringMainnet(receiverPKID[:]), types.PkToStringMainnet(senderPKID[:]))
		}
		expectedDiamondKeyLen := 1 + 2*btcec.PubKeyBytesLenCompressed + types.HashSizeBytes
		if len(keyBytes) != expectedDiamondKeyLen {
			return nil, fmt.Errorf(
...
func _keyForBitcoinBurnTxID(bitcoinBurnTxID *types.BlockHash) []byte {
...
func DbPutBitcoinBurnTxIDWithTxn(txn *badger.Txn, bitcoinBurnTxID *types.BlockHash) error {
	return txn.Set(_keyForBitcoinBurnTxID(bitcoinBurnTxID), []byte{})
}

func DbExistsBitcoinBurnTxIDWithTxn(txn *badger.Txn, bitcoinBurnTxID *types.BlockHash) bool {
...
func DbExistsBitcoinBurnTxID(db *badger.DB, bitcoinBurnTxID *types.BlockHash) bool {
	var exists bool
	db.View(func(txn *badger.Txn) error {
...
func DbDeleteBitcoinBurnTxIDWithTxn(txn *badger.Txn, bitcoinBurnTxID *types.BlockHash) error {
	return txn.Delete(_keyForBitcoinBurnTxID(bitcoinBurnTxID))
}

func DbGetAllBitcoinBurnTxIDs(handle *badger.DB) (_bitcoinBurnTxIDs []*types.BlockHash) {
	keysFound, _ := _enumerateKeysForPrefix(handle, _PrefixBitcoinBurnTxIDs)
	bitcoinBurnTxIDs := []*types.BlockHash{}
	for _, key := range keysFound {
		bbtxid := &types.BlockHash{}
		copy(bbtxid[:], key[1:])
...
func _getBlockHashForPrefixWithTxn(txn *badger.Txn, prefix []byte) *types.BlockHash {
	var ret types.BlockHash
...
func _getBlockHashForPrefix(handle *badger.DB, prefix []byte) *types.BlockHash {
	var ret *types.BlockHash
...
func DbPutGlobalParamsEntry(handle *badger.DB, globalParamsEntry block_view.GlobalParamsEntry) error {
	return handle.Update(func(txn *badger.Txn) error {
		return DbPutGlobalParamsEntryWithTxn(txn, globalParamsEntry)
	})
}

func DbPutGlobalParamsEntryWithTxn(txn *badger.Txn, globalParamsEntry block_view.GlobalParamsEntry) error {
...
func DbGetGlobalParamsEntryWithTxn(txn *badger.Txn) *block_view.GlobalParamsEntry {
	globalParamsEntryItem, err := txn.Get(_KeyGlobalParams)
	if err != nil {
		return &types.InitialGlobalParamsEntry
	}
	globalParamsEntryObj := &block_view.GlobalParamsEntry{}
...
func DbGetGlobalParamsEntry(handle *badger.DB) *block_view.GlobalParamsEntry {
	var globalParamsEntry *block_view.GlobalParamsEntry
	handle.View(func(txn *badger.Txn) error {
...
func DbGetUSDCentsPerBitcoinExchangeRateWithTxn(txn *badger.Txn) uint64 {
	...
	if err != nil {
		return types.InitialUSDCentsPerBitcoinExchangeRate
	}
...
func _SerializeUtxoKey(utxoKey *types.UtxoKey) []byte {
...
func _DbKeyForUtxoKey(utxoKey *types.UtxoKey) []byte {
...
// this function.
func _UtxoKeyFromDbKey(utxoDbKey []byte) *types.UtxoKey {
	// Read in the TxID, which is at the beginning.
	txIDBytes := utxoDbKey[:types.HashSizeBytes]
	txID := types.BlockHash{}
	copy(txID[:], txIDBytes)
	// Read in the index, which is encoded as a bigint at the end.
	indexBytes := utxoDbKey[types.HashSizeBytes:]
	indexValue := binary.BigEndian.Uint32(indexBytes)
	return &types.UtxoKey{
		Index: indexValue,
		TxID:  txID,
	}
}

func _DbBufForUtxoEntry(utxoEntry *block_view.UtxoEntry) []byte {
...
func PutUtxoEntryForUtxoKeyWithTxn(txn *badger.Txn, utxoKey *types.UtxoKey, utxoEntry *block_view.UtxoEntry) error {
	return txn.Set(_DbKeyForUtxoKey(utxoKey), _DbBufForUtxoEntry(utxoEntry))
}

func DbGetUtxoEntryForUtxoKeyWithTxn(txn *badger.Txn, utxoKey *types.UtxoKey) *block_view.UtxoEntry {
	var ret block_view.UtxoEntry
...
func DbGetUtxoEntryForUtxoKey(handle *badger.DB, utxoKey *types.UtxoKey) *block_view.UtxoEntry {
	var ret *block_view.UtxoEntry
	handle.View(func(txn *badger.Txn) error {
...
func DeleteUtxoEntryForKeyWithTxn(txn *badger.Txn, utxoKey *types.UtxoKey) error {
	return txn.Delete(_DbKeyForUtxoKey(utxoKey))
}

func DeletePubKeyUtxoKeyMappingWithTxn(txn *badger.Txn, publicKey []byte, utxoKey *types.UtxoKey) error {
	if len(publicKey) != btcec.PubKeyBytesLenCompressed {
...
func DbBufForUtxoKey(utxoKey *types.UtxoKey) []byte {
	utxoKeyBuf := bytes.NewBuffer([]byte{})
	gob.NewEncoder(utxoKeyBuf).Encode(utxoKey)
	return utxoKeyBuf.Bytes()
}

func PutPubKeyUtxoKeyWithTxn(txn *badger.Txn, publicKey []byte, utxoKey *types.UtxoKey) error {
	if len(publicKey) != btcec.PubKeyBytesLenCompressed {
...
func DbGetUtxosForPubKey(publicKey []byte, handle *badger.DB) ([]*block_view.UtxoEntry, error) {
	...
	utxoEntriesFound := []*block_view.UtxoEntry{}
	err := handle.View(func(txn *badger.Txn) error {
		// Start by looping through to find all the UtxoKeys.
		utxoKeysFound := []*types.UtxoKey{}
		opts := badger.DefaultIteratorOptions
		...
		for nodeIterator.Seek(prefix); nodeIterator.ValidForPrefix(prefix); nodeIterator.Next() {
			...
			if len(utxoKeyBytes) != types.HashSizeBytes+4 {
				return fmt.Errorf("Problem reading <pk, utxoKey> mapping; key size %d "+
					...
					"Key found: %#v", len(pkUtxoKey), len(_PrefixPubKeyUtxoKey), len(publicKey), types.HashSizeBytes+4, len(prefix)+types.HashSizeBytes+4, pkUtxoKey)
			}
...
func DeleteUnmodifiedMappingsForUtxoWithTxn(txn *badger.Txn, utxoKey *types.UtxoKey) error {
...
func PutMappingsForUtxoWithTxn(txn *badger.Txn, utxoKey *types.UtxoKey, utxoEntry *block_view.UtxoEntry) error {
...
func _DecodeUtxoOperations(data []byte) ([][]*block_view.UtxoOperation, error) {
	ret := [][]*block_view.UtxoOperation{}
...
func _EncodeUtxoOperations(utxoOp [][]*block_view.UtxoOperation) []byte {
	opBuf := bytes.NewBuffer([]byte{})
	gob.NewEncoder(opBuf).Encode(utxoOp)
	return opBuf.Bytes()
}

func _DbKeyForUtxoOps(blockHash *types.BlockHash) []byte {
	return append(append([]byte{}, _PrefixBlockHashToUtxoOperations...), blockHash[:]...)
}

func GetUtxoOperationsForBlockWithTxn(txn *badger.Txn, blockHash *types.BlockHash) ([][]*block_view.UtxoOperation, error) {
	var retOps [][]*block_view.UtxoOperation
...
func GetUtxoOperationsForBlock(handle *badger.DB, blockHash *types.BlockHash) ([][]*block_view.UtxoOperation, error) {
	var ops [][]*block_view.UtxoOperation
	err := handle.View(func(txn *badger.Txn) error {
		var err error
...
func PutUtxoOperationsForBlockWithTxn(txn *badger.Txn, blockHash *types.BlockHash, utxoOpsForBlock [][]*block_view.UtxoOperation) error {
	return txn.Set(_DbKeyForUtxoOps(blockHash), _EncodeUtxoOperations(utxoOpsForBlock))
}

func DeleteUtxoOperationsForBlockWithTxn(txn *badger.Txn, blockHash *types.BlockHash) error {
	return txn.Delete(_DbKeyForUtxoOps(blockHash))
}

func SerializeBlockNode(blockNode *types.BlockNode) ([]byte, error) {
	data := []byte{}
	...
	// Height
	data = append(data, network.UintToBuf(uint64(blockNode.Height))...)
	...
	// CumWork
	data = append(data, types.BigintToHash(blockNode.CumWork)[:]...)
	...
	data = append(data, network.IntToBuf(int64(len(serializedHeader)))...)
	data = append(data, serializedHeader...)

	// Status
	// It's assumed this field is one byte long.
	data = append(data, network.UintToBuf(uint64(blockNode.Status))...)

	return data, nil
}

func DeserializeBlockNode(data []byte) (*types.BlockNode, error) {
	blockNode := types.NewBlockNode(
		nil,                // Parent
		&types.BlockHash{}, // Hash
		0,                  // Height
		&types.BlockHash{}, // DifficultyTarget
		nil,                // CumWork
		nil,                // Header
		types.StatusNone,   // Status
	...
	// Height
	height, err := network.ReadUvarint(rr)
	if err != nil {
	...
	blockNode.CumWork = types.HashToBigint(&tmp)

	// Header
	payloadLen, err := network.ReadVarint(rr)
	if err != nil {
	...
	blockNode.Header = network.NewMessage(network.MsgTypeHeader).(*types.MsgDeSoHeader)
	...
	// Status
	status, err := network.ReadUvarint(rr)
	if err != nil {
		return nil, errors.Wrapf(err, "DeserializeBlockNode: Problem decoding Status")
	}
	blockNode.Status = types.BlockStatus(uint32(status))
...
func DbGetBestHash(handle *badger.DB, chainType ChainType) *types.BlockHash {
...
func PutBestHashWithTxn(txn *badger.Txn, bh *types.BlockHash, chainType ChainType) error {
	prefix := _prefixForChainType(chainType)
	if len(prefix) == 0 {
...
func PutBestHash(bh *types.BlockHash, handle *badger.DB, chainType ChainType) error {
	return handle.Update(func(txn *badger.Txn) error {
		return PutBestHashWithTxn(txn, bh, chainType)
	})
}

func BlockHashToBlockKey(blockHash *types.BlockHash) []byte {
	return append(append([]byte{}, _PrefixBlockHashToBlock...), blockHash[:]...)
}

func PublicKeyBlockHashToBlockRewardKey(publicKey []byte, blockHash *types.BlockHash) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func GetBlockWithTxn(txn *badger.Txn, blockHash *types.BlockHash) *network.MsgDeSoBlock {
	hashKey := BlockHashToBlockKey(blockHash)
	var blockRet *network.MsgDeSoBlock
	...
	err = item.Value(func(valBytes []byte) error {
		ret := network.NewMessage(network.MsgTypeBlock).(*network.MsgDeSoBlock)
...
func GetBlock(blockHash *types.BlockHash, handle *badger.DB) (*network.MsgDeSoBlock, error) {
	hashKey := BlockHashToBlockKey(blockHash)
	var blockRet *network.MsgDeSoBlock
	err := handle.View(func(txn *badger.Txn) error {
		item, err := txn.Get(hashKey)
...
func PutBlockWithTxn(txn *badger.Txn, desoBlock *network.MsgDeSoBlock) error {
	...
	if blockRewardTxn.TxnMeta.GetTxnType() != network.TxnTypeBlockReward {
	...
	pubKeyToBlockRewardMap := make(map[block_view.PkMapKey]uint64)
	for _, bro := range desoBlock.Txns[0].TxOutputs {
		pkMapKey := block_view.MakePkMapKey(bro.PublicKey)
...
func PutBlock(desoBlock *network.MsgDeSoBlock, handle *badger.DB) error {
...
func DbGetBlockRewardForPublicKeyBlockHashWithTxn(txn *badger.Txn, publicKey []byte, blockHash *types.BlockHash,
) (_balance uint64, _err error) {
...
func DbGetBlockRewardForPublicKeyBlockHash(db *badger.DB, publicKey []byte, blockHash *types.BlockHash,
) (_balance uint64, _err error) {
	ret := uint64(0)
	dbErr := db.View(func(txn *badger.Txn) error {
		var err error
...
func _heightHashToNodeIndexKey(height uint32, hash *types.BlockHash, bitcoinNodes bool) []byte {
...
func GetHeightHashToNodeInfoWithTxn(
	txn *badger.Txn, height uint32, hash *types.BlockHash, bitcoinNodes bool) *types.BlockNode {
...
func GetHeightHashToNodeInfo(
	handle *badger.DB, height uint32, hash *types.BlockHash, bitcoinNodes bool) *types.BlockNode {
...
func PutHeightHashToNodeInfoWithTxn(txn *badger.Txn, node *types.BlockNode, bitcoinNodes bool) error {
...
func PutHeightHashToNodeInfo(node *types.BlockNode, handle *badger.DB, bitcoinNodes bool) error {
	err := handle.Update(func(txn *badger.Txn) error {
...
func DbDeleteHeightHashToNodeInfoWithTxn(
	node *types.BlockNode, txn *badger.Txn, bitcoinNodes bool) error {

	return txn.Delete(_heightHashToNodeIndexKey(node.Height, node.Hash, bitcoinNodes))
}

func DbBulkDeleteHeightHashToNodeInfo(
	nodes []*types.BlockNode, handle *badger.DB, bitcoinNodes bool) error {
...
// block.
func InitDbWithDeSoGenesisBlock(params *types.DeSoParams, handle *badger.DB, eventManager *lib.EventManager) error {
	...
	diffTarget := types.MustDecodeHexBlockHash(params.MinDifficultyTargetHex)
	blockHash := types.MustDecodeHexBlockHash(params.GenesisBlockHashHex)
	genesisNode := types.NewBlockNode(
		nil, // Parent
		blockHash,
		0, // Height
		diffTarget,
		types.BytesToBigint(types.ExpectedWorkForBlockHash(diffTarget)[:]), // CumWork
		genesisBlock.Header, // Header
		types.StatusHeaderValidated|types.StatusBlockProcessed|types.StatusBlockStored|types.StatusBlockValidated, // Status
	)
	...
	if err := DbPutGlobalParamsEntry(handle, types.InitialGlobalParamsEntry); err != nil {
	...
	utxoView, err := block_view.NewUtxoView(handle, params, nil)
	if err != nil {
		return fmt.Errorf(
	...
	for index, txOutput := range params.SeedBalances {
		...
		utxoEntry := block_view.UtxoEntry{
			...
			UtxoType: block_view.UtxoTypeOutput,
			UtxoKey:  &outputKey,
		}
	...
	utxoOpsForBlock := [][]*block_view.UtxoOperation{}
	for txnIndex, txnHex := range params.SeedTxns {
		...
		var utxoOpsForTxn []*block_view.UtxoOperation
	...
	// state of the view.
	if eventManager != nil {
		eventManager.blockConnected(&lib.BlockEvent{
			Block:    genesisBlock,
			UtxoView: utxoView,
...
func GetBlockIndex(handle *badger.DB, bitcoinNodes bool) (map[types.BlockHash]*types.BlockNode, error) {
	blockIndex := make(map[types.BlockHash]*types.BlockNode)
	...
	err := handle.View(func(txn *badger.Txn) error {
		...
		for nodeIterator.Seek(prefix); nodeIterator.ValidForPrefix(prefix); nodeIterator.Next() {
			...
			if blockNode.Height == 0 || (*blockNode.Header.PrevBlockHash == types.BlockHash{}) {
				continue
			}
...
func GetBestChain(tipNode *types.BlockNode, blockIndex map[types.BlockHash]*types.BlockNode) ([]*types.BlockNode, error) {
	reversedBestChain := []*types.BlockNode{}
	for tipNode != nil {
		if (tipNode.Status&types.StatusBlockValidated) == 0 &&
			(tipNode.Status&types.StatusBitcoinHeaderValidated) == 0 {
	...
	bestChain := make([]*types.BlockNode, len(reversedBestChain))
...
func DbGetTxindexTip(handle *badger.DB) *types.BlockHash {
	return _getBlockHashForPrefix(handle, _KeyTransactionIndexTip)
}

func DbPutTxindexTipWithTxn(dbTxn *badger.Txn, tipHash *types.BlockHash) error {
	return dbTxn.Set(_KeyTransactionIndexTip, tipHash[:])
}

func DbPutTxindexTip(handle *badger.DB, tipHash *types.BlockHash) error {
	return handle.Update(func(txn *badger.Txn) error {
...
func DbGetTxindexTxnsForPublicKeyWithTxn(dbTxn *badger.Txn, publicKey []byte) []*types.BlockHash {
...
func DbGetTxindexTxnsForPublicKey(handle *badger.DB, publicKey []byte) []*types.BlockHash {
	txIDs := []*types.BlockHash{}
...
func _DbGetTxindexNextIndexForPublicKeyWithTxn(txn *badger.Txn, publicKey []byte) *uint64 {
	...
	nextIndexVal, bytesRead := network.Uvarint(valBytes)
	if bytesRead <= 0 {
		return nil
	}
	return &nextIndexVal
...
func DbPutTxindexNextIndexForPublicKeyWithTxn(txn *badger.Txn, publicKey []byte, nextIndex uint64) error {
	key := _DbTxindexPublicKeyNextIndexPrefix(publicKey)
	valBuf := network.UintToBuf(nextIndex)
...
func DbPutTxindexPublicKeyToTxnMappingSingleWithTxn(
	dbTxn *badger.Txn, publicKey []byte, txID *types.BlockHash) error {
...
func DbTxindexTxIDKey(txID *types.BlockHash) []byte {
...
func DBCheckTxnExistenceWithTxn(txn *badger.Txn, txID *types.BlockHash) bool {
...
func DbCheckTxnExistence(handle *badger.DB, txID *types.BlockHash) bool {
	var exists bool
	handle.View(func(txn *badger.Txn) error {
...
func DbGetTxindexTransactionRefByTxIDWithTxn(txn *badger.Txn, txID *types.BlockHash) *TransactionMetadata {
	key := DbTxindexTxIDKey(txID)
...
func DbGetTxindexTransactionRefByTxID(handle *badger.DB, txID *types.BlockHash) *TransactionMetadata {
...
func DbPutTxindexTransactionWithTxn(
	txn *badger.Txn, txID *types.BlockHash, txnMeta *TransactionMetadata) error {
...
func DbPutTxindexTransaction(
	handle *badger.DB, txID *types.BlockHash, txnMeta *TransactionMetadata) error {
...
func _getPublicKeysForTxn(
	txn *network.MsgDeSoTxn, txnMeta *TransactionMetadata, params *types.DeSoParams) map[block_view.PkMapKey]bool {

	// Collect the public keys in the transaction.
	publicKeys := make(map[block_view.PkMapKey]bool)
	...
	{
		res, _, err := types.Base58CheckDecode(txnMeta.TransactorPublicKeyBase58Check)
		if err != nil {
			...
			publicKeys[block_view.MakePkMapKey(res)] = true
		}
	}

	// Add each AffectedPublicKey
	for _, affectedPk := range txnMeta.AffectedPublicKeys {
		res, _, err := types.Base58CheckDecode(affectedPk.PublicKeyBase58Check)
		if err != nil {
...
func DbPutTxindexTransactionMappingsWithTxn(
	dbTx *badger.Txn, txn *network.MsgDeSoTxn, params *types.DeSoParams, txnMeta *TransactionMetadata) error {
...
func DbPutTxindexTransactionMappings(
	handle *badger.DB, desoTxn *network.MsgDeSoTxn, params *types.DeSoParams, txnMeta *TransactionMetadata) error {
...
func DbDeleteTxindexTransactionMappingsWithTxn(
	dbTxn *badger.Txn, txn *network.MsgDeSoTxn, params *types.DeSoParams) error {
...
func DbDeleteTxindexTransactionMappings(
	handle *badger.DB, txn *network.MsgDeSoTxn, params *types.DeSoParams) error {
...
func DbGetTxindexFullTransactionByTxID(
	txindexDBHandle *badger.DB, blockchainDBHandle *badger.DB, txID *types.BlockHash) (
	_txn *network.MsgDeSoTxn, _txnMeta *TransactionMetadata) {

	var txnFound *network.MsgDeSoTxn
...
func _dbKeyForPostEntryHash(postHash *types.BlockHash) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
	prefixCopy := append([]byte{}, _PrefixPostHashToPostEntry...)
	key := append(prefixCopy, postHash[:]...)
	return key
}
func _dbKeyForPublicKeyPostHash(publicKey []byte, postHash *types.BlockHash) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbKeyForPosterPublicKeyTimestampPostHash(publicKey []byte, timestampNanos uint64, postHash *types.BlockHash) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbKeyForTstampPostHash(tstampNanos uint64, postHash *types.BlockHash) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbKeyForCreatorBpsPostHash(creatorBps uint64, postHash *types.BlockHash) []byte {
	key := append([]byte{}, _PrefixCreatorBpsPostHash...)
	key = append(key, EncodeUint64(creatorBps)...)
	key = append(key, postHash[:]...)
	return key
}
func _dbKeyForStakeMultipleBpsPostHash(stakeMultipleBps uint64, postHash *types.BlockHash) []byte {
...
func _dbKeyForCommentParentStakeIDToPostHash(
	stakeID []byte, tstampNanos uint64, postHash *types.BlockHash) []byte {
...
func DBGetPostEntryByPostHashWithTxn(
	txn *badger.Txn, postHash *types.BlockHash) *block_view.PostEntry {

	key := _dbKeyForPostEntryHash(postHash)
	postEntryObj := &block_view.PostEntry{}
...
func DBGetPostEntryByPostHash(db *badger.DB, postHash *types.BlockHash) *block_view.PostEntry {
	var ret *block_view.PostEntry
	db.View(func(txn *badger.Txn) error {
...
func DBDeletePostEntryMappingsWithTxn(
	txn *badger.Txn, postHash *types.BlockHash, params *types.DeSoParams) error {
	...
	// get code re-use.
	isComment := len(postEntry.ParentStakeID) == types.HashSizeBytes
	if isComment {
	...
	if block_view.IsVanillaRepost(postEntry) {
		if err := txn.Delete(
	...
	} else if block_view.IsQuotedRepost(postEntry) {
...
func DBDeletePostEntryMappings(
	handle *badger.DB, postHash *types.BlockHash, params *types.DeSoParams) error {
...
func DBPutPostEntryMappingsWithTxn(
	txn *badger.Txn, postEntry *block_view.PostEntry, params *types.DeSoParams) error {
	...
	if isComment {
		...
		// key.
		extendedStakeID := append([]byte{}, postEntry.ParentStakeID...)
		if len(extendedStakeID) == types.HashSizeBytes {
			extendedStakeID = append(extendedStakeID, 0x00)
		}
	...
	if block_view.IsVanillaRepost(postEntry) {
		repostEntry := block_view.RepostEntry{
...
func DBPutPostEntryMappings(handle *badger.DB, postEntry *block_view.PostEntry, params *types.DeSoParams) error {
...
func DBGetAllPostsAndCommentsForPublicKeyOrderedByTimestamp(
	...
	_tstamps []uint64, _postAndCommentHashes []*types.BlockHash, _postAndCommentEntries []*block_view.PostEntry, _err error) {

	tstampsFetched := []uint64{}
	postAndCommentHashesFetched := []*types.BlockHash{}
	postAndCommentEntriesFetched := []*block_view.PostEntry{}
	...
	err := handle.View(func(txn *badger.Txn) error {
		...
		for it.Seek(prefix); it.ValidForPrefix(dbPrefixx); it.Next() {
			...
			publicKeySizeBytes := types.HashSizeBytes + 1
...
func DBGetAllPostsByTstamp(handle *badger.DB, fetchEntries bool) (
	_tstamps []uint64, _postHashes []*types.BlockHash, _postEntries []*block_view.PostEntry, _err error) {

	tstampsFetched := []uint64{}
	postHashesFetched := []*types.BlockHash{}
	postEntriesFetched := []*block_view.PostEntry{}
...
func DBGetCommentPostHashesForParentStakeID(
	...
	_tstamps []uint64, _commentPostHashes []*types.BlockHash, _commentPostEntryes []*block_view.PostEntry, _err error) {

	tstampsFetched := []uint64{}
	commentPostHashes := []*types.BlockHash{}
	commentEntriesFetched := []*block_view.PostEntry{}
	...
	err := handle.View(func(txn *badger.Txn) error {
		...
		for it.Seek(prefix); it.ValidForPrefix(dbPrefixx); it.Next() {
			...
			uint64BytesLen := len(maxBigEndianUint64Bytes)
			if len(stakeIDTstampPostHashKey) != btcec.PubKeyBytesLenCompressed+uint64BytesLen+types.HashSizeBytes {
				return fmt.Errorf("DBGetCommentPostHashesForParentStakeID: Invalid key "+
					...
					btcec.PubKeyBytesLenCompressed+uint64BytesLen+types.HashSizeBytes)
			}
			...
			commentPostHash := &types.BlockHash{}
...
// =======================================================================================
func _dbKeyForNFTPostHashSerialNumber(nftPostHash *types.BlockHash, serialNumber uint64) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbKeyForPKIDIsForSaleBidAmountNanosNFTPostHashSerialNumber(pkid *types.PKID, isForSale bool, bidAmountNanos uint64, nftPostHash *types.BlockHash, serialNumber uint64) []byte {
	prefixCopy := append([]byte{}, _PrefixPKIDIsForSaleBidAmountNanosPostHashSerialNumberToNFTEntry...)
	key := append(prefixCopy, pkid[:]...)
	key = append(key, network.BoolToByte(isForSale))
...
func DBGetNFTEntryByPostHashSerialNumberWithTxn(
	txn *badger.Txn, postHash *types.BlockHash, serialNumber uint64) *block_view.NFTEntry {

	key := _dbKeyForNFTPostHashSerialNumber(postHash, serialNumber)
	nftEntryObj := &block_view.NFTEntry{}
...
func DBGetNFTEntryByPostHashSerialNumber(db *badger.DB, postHash *types.BlockHash, serialNumber uint64) *block_view.NFTEntry {
	var ret *block_view.NFTEntry
	db.View(func(txn *badger.Txn) error {
...
func DBDeleteNFTMappingsWithTxn(txn *badger.Txn, nftPostHash *types.BlockHash, serialNumber uint64) error {
...
func DBDeleteNFTMappings(
	handle *badger.DB, postHash *types.BlockHash, serialNumber uint64) error {
...
func DBPutNFTEntryMappingsWithTxn(txn *badger.Txn, nftEntry *block_view.NFTEntry) error {
...
func DBPutNFTEntryMappings(handle *badger.DB, nftEntry *block_view.NFTEntry) error {
...
func DBGetNFTEntriesForPostHash(handle *badger.DB, nftPostHash *types.BlockHash) (_nftEntries []*block_view.NFTEntry) {
	nftEntries := []*block_view.NFTEntry{}
	...
	for _, byteString := range entryByteStringsFound {
		currentEntry := &block_view.NFTEntry{}
...
func DBGetNFTEntryByNFTOwnershipDetailsWithTxn(
	txn *badger.Txn, ownerPKID *types.PKID, isForSale bool, bidAmountNanos uint64, postHash *types.BlockHash, serialNumber uint64) *block_view.NFTEntry {
...
func DBGetNFTEntryByNFTOwnershipDetails(db *badger.DB, ownerPKID *types.PKID, isForSale bool, bidAmountNanos uint64, postHash *types.BlockHash, serialNumber uint64) *block_view.NFTEntry {
	var ret *block_view.NFTEntry
	db.View(func(txn *badger.Txn) error {
...
func DBGetNFTEntriesForPKID(handle *badger.DB, ownerPKID *types.PKID) (_nftEntries []*block_view.NFTEntry) {
	nftEntries := []*block_view.NFTEntry{}
...
// outside of the protocol layer once update to the creation of TxIndex are complete.
// =======================================================================================
func _dbKeyForPostHashSerialNumberToAcceptedBidEntries(nftPostHash *types.BlockHash, serialNumber uint64) []byte {
...
func DBPutAcceptedNFTBidEntriesMappingWithTxn(txn *badger.Txn, nftKey block_view.NFTKey, nftBidEntries *[]*block_view.NFTBidEntry) error {
	nftDataBuf := bytes.NewBuffer([]byte{})
...
func DBPutAcceptedNFTBidEntriesMapping(handle *badger.DB, nftKey block_view.NFTKey, nftBidEntries *[]*block_view.NFTBidEntry) error {
	return handle.Update(func(txn *badger.Txn) error {
...
func DBGetAcceptedNFTBidEntriesByPostHashSerialNumberWithTxn(
	txn *badger.Txn, postHash *types.BlockHash, serialNumber uint64) *[]*block_view.NFTBidEntry {

	key := _dbKeyForPostHashSerialNumberToAcceptedBidEntries(postHash, serialNumber)
	nftBidEntriesObj := &[]*block_view.NFTBidEntry{}
...
func DBGetAcceptedNFTBidEntriesByPostHashSerialNumber(db *badger.DB, postHash *types.BlockHash, serialNumber uint64) *[]*block_view.NFTBidEntry {
	var ret *[]*block_view.NFTBidEntry
	db.View(func(txn *badger.Txn) error {
...
func DBDeleteAcceptedNFTBidEntriesMappingsWithTxn(txn *badger.Txn, nftPostHash *types.BlockHash, serialNumber uint64) error {
...
func _dbKeyForNFTPostHashSerialNumberBidNanosBidderPKID(bidEntry *block_view.NFTBidEntry) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbKeyForNFTBidderPKIDPostHashSerialNumber(
	bidderPKID *types.PKID, nftPostHash *types.BlockHash, serialNumber uint64) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbSeekKeyForNFTBids(nftHash *types.BlockHash, serialNumber uint64) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
	prefixCopy := append([]byte{}, _PrefixPostHashSerialNumberBidNanosBidderPKID...)
...
func DBGetNFTBidEntryForNFTBidKeyWithTxn(txn *badger.Txn, nftBidKey *block_view.NFTBidKey) *block_view.NFTBidEntry {
	...
	nftBidEntry := &block_view.NFTBidEntry{
...
func DBGetNFTBidEntryForNFTBidKey(db *badger.DB, nftBidKey *block_view.NFTBidKey) *block_view.NFTBidEntry {
	var ret *block_view.NFTBidEntry
	db.View(func(txn *badger.Txn) error {
...
func DBDeleteNFTBidMappingsWithTxn(txn *badger.Txn, nftBidKey *block_view.NFTBidKey) error {
...
func DBDeleteNFTBidMappings(handle *badger.DB, nftBidKey *block_view.NFTBidKey) error {
...
func DBPutNFTBidEntryMappingsWithTxn(txn *badger.Txn, nftBidEntry *block_view.NFTBidEntry) error {
...
func DBPutNFTBidEntryMappings(handle *badger.DB, nftEntry *block_view.NFTBidEntry) error {
...
func DBGetNFTBidEntriesForPKID(handle *badger.DB, bidderPKID *types.PKID) (_nftBidEntries []*block_view.NFTBidEntry) {
	nftBidEntries := []*block_view.NFTBidEntry{}
	{
		...
		for ii, keyFound := range keysFound {
			...
			postHashEndIdx := postHashStartIdx + types.HashSizeBytes // Add the length of the bid amount (uint64).

			// Cut the bid amount out of the key and decode.
			postHashBytes := keyFound[postHashStartIdx:postHashEndIdx]

			nftHash := &types.BlockHash{}
			...
			currentEntry := &block_view.NFTBidEntry{
				NFTPostHash:    nftHash,
...
func DBGetNFTBidEntries(handle *badger.DB, nftPostHash *types.BlockHash, serialNumber uint64,
) (_nftBidEntries []*block_view.NFTBidEntry) {
	nftBidEntries := []*block_view.NFTBidEntry{}
	{
		...
		for _, keyFound := range keysFound {
			bidAmountStartIdx := 1 + types.HashSizeBytes + 8 // The length of prefix + the post hash + the serial #.
			...
			bidderPKID := types.PublicKeyToPKID(bidderPKIDBytes)
...
func DBGetNFTBidEntriesPaginated(
	...
	nftHash *types.BlockHash,
	serialNumber uint64,
	startEntry *block_view.NFTBidEntry,
	limit int,
	reverse bool,
) (_bidEntries []*block_view.NFTBidEntry) {
	...
	maxKeyLen := 1 + types.HashSizeBytes + 16 + btcec.PubKeyBytesLenCompressed
	...
	var bidEntries []*block_view.NFTBidEntry
	for _, keyBytes := range keysBytes {
		serialNumStartIdx := 1 + types.HashSizeBytes
		...
		bidderPKID := &types.PKID{}
		copy(bidderPKID[:], bidderPKIDBytes)

		bidEntry := &block_view.NFTBidEntry{
			NFTPostHash:    nftHash,
			SerialNumber:   serialNumber,
...
func _dbKeyForOwnerToDerivedKeyMapping(
	ownerPublicKey types.PublicKey, derivedPublicKey types.PublicKey) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func _dbSeekPrefixForDerivedKeyMappings(
	ownerPublicKey types.PublicKey) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
	prefixCopy := append([]byte{}, _PrefixAuthorizeDerivedKey...)
	key := append(prefixCopy, ownerPublicKey[:]...)
	return key
...
func DBPutDerivedKeyMappingWithTxn(
	txn *badger.Txn, ownerPublicKey types.PublicKey, derivedPublicKey types.PublicKey, derivedKeyEntry *block_view.DerivedKeyEntry) error {
...
func DBPutDerivedKeyMapping(
	handle *badger.DB, ownerPublicKey types.PublicKey, derivedPublicKey types.PublicKey, derivedKeyEntry *block_view.DerivedKeyEntry) error {
...
func DBGetOwnerToDerivedKeyMappingWithTxn(
	txn *badger.Txn, ownerPublicKey types.PublicKey, derivedPublicKey types.PublicKey) *block_view.DerivedKeyEntry {
	...
	derivedKeyEntry := &block_view.DerivedKeyEntry{}
...
func DBGetOwnerToDerivedKeyMapping(
	db *badger.DB, ownerPublicKey types.PublicKey, derivedPublicKey types.PublicKey) *block_view.DerivedKeyEntry {

	var derivedKeyEntry *block_view.DerivedKeyEntry
	db.View(func(txn *badger.Txn) error {
...
func DBDeleteDerivedKeyMappingWithTxn(
	txn *badger.Txn, ownerPublicKey types.PublicKey, derivedPublicKey types.PublicKey) error {
	...
	if err := txn.Delete(_dbKeyForOwnerToDerivedKeyMapping(ownerPublicKey, derivedPublicKey)); err != nil {
		return errors.Wrapf(err, "DBDeleteDerivedKeyMappingWithTxn: Deleting "+
			...
			types.PkToStringMainnet(ownerPublicKey[:]), types.PkToStringMainnet(derivedPublicKey[:]))
	}
...
func DBDeleteDerivedKeyMapping(
	handle *badger.DB, ownerPublicKey types.PublicKey, derivedPublicKey types.PublicKey) error {
	return handle.Update(func(txn *badger.Txn) error {
		return DBDeleteDerivedKeyMappingWithTxn(txn, ownerPublicKey, derivedPublicKey)
	})
}

func DBGetAllOwnerToDerivedKeyMappings(handle *badger.DB, ownerPublicKey types.PublicKey) (
	_entries []*block_view.DerivedKeyEntry, _err error) {

	prefix := _dbSeekPrefixForDerivedKeyMappings(ownerPublicKey)
	_, valsFound := _enumerateKeysForPrefix(handle, prefix)

	var derivedEntries []*block_view.DerivedKeyEntry
...
// Profile code
// ======================================================================================
func _dbKeyForPKIDToProfileEntry(pkid *types.PKID) []byte {
...
func _dbKeyForCreatorDeSoLockedNanosCreatorPKID(desoLockedNanos uint64, pkid *types.PKID) []byte {
...
func DBGetPKIDForUsernameWithTxn(
	txn *badger.Txn, username []byte) *types.PKID {
	...
	return types.PublicKeyToPKID(pkidBytes)
}

func DBGetPKIDForUsername(db *badger.DB, username []byte) *types.PKID {
	var ret *types.PKID
	db.View(func(txn *badger.Txn) error {
...
func DBGetProfileEntryForUsernameWithTxn(
	txn *badger.Txn, username []byte) *block_view.ProfileEntry {
...
func DBGetProfileEntryForUsername(db *badger.DB, username []byte) *block_view.ProfileEntry {
	var ret *block_view.ProfileEntry
	db.View(func(txn *badger.Txn) error {
...
func DBGetProfileEntryForPKIDWithTxn(
	txn *badger.Txn, pkid *types.PKID) *block_view.ProfileEntry {

	key := _dbKeyForPKIDToProfileEntry(pkid)
	profileEntryObj := &block_view.ProfileEntry{}
	profileEntryItem, err := txn.Get(key)
	if err != nil {
		return nil
	}
...
func DBGetProfileEntryForPKID(db *badger.DB, pkid *types.PKID) *block_view.ProfileEntry {
	var ret *block_view.ProfileEntry
	db.View(func(txn *badger.Txn) error {
...
func DBDeleteProfileEntryMappingsWithTxn(
	txn *badger.Txn, pkid *types.PKID, params *types.DeSoParams) error {
...
func DBDeleteProfileEntryMappings(
	handle *badger.DB, pkid *types.PKID, params *types.DeSoParams) error {
...
func DBPutProfileEntryMappingsWithTxn(
	txn *badger.Txn, profileEntry *block_view.ProfileEntry, pkid *types.PKID, params *types.DeSoParams) error {
	...
	if err := txn.Set(_dbKeyForPKIDToProfileEntry(pkid), profileDataBuf.Bytes()); err != nil {
		...
		return errors.Wrapf(err, "DbPutProfileEntryMappingsWithTxn: Problem "+
			"adding mapping for profile: %v", types.PkToString(pkid[:], params))
	}
...
func DBPutProfileEntryMappings(
	handle *badger.DB, profileEntry *block_view.ProfileEntry, pkid *types.PKID, params *types.DeSoParams) error {
...
func DBGetAllProfilesByCoinValue(handle *badger.DB, fetchEntries bool) (
	_lockedDeSoNanos []uint64, _profilePublicKeys []*types.PKID,
	_profileEntries []*block_view.ProfileEntry, _err error) {

	lockedDeSoNanosFetched := []uint64{}
	profilePublicKeysFetched := []*types.PKID{}
	profileEntriesFetched := []*block_view.ProfileEntry{}
	...
	err := handle.View(func(txn *badger.Txn) error {
		...
		for it.Seek(prefix); it.ValidForPrefix(dbPrefixx); it.Next() {
			...
			profilePublicKeysFetched = append(profilePublicKeysFetched, types.PublicKeyToPKID(profilePKID))
		}
		return nil
	})
	if err != nil {
	...
	for _, profilePKID := range profilePublicKeysFetched {
		...
		if profileEntry == nil {
			return nil, nil, nil, fmt.Errorf("DBGetAllProfilesByLockedDeSo: "+
				...
				types.PkToStringBoth(profilePKID[:]))
		}
...
// =====================================================================================
func _dbKeyForHODLerPKIDCreatorPKIDToBalanceEntry(hodlerPKID *types.PKID, creatorPKID *types.PKID) []byte {
	key := append([]byte{}, _PrefixHODLerPKIDCreatorPKIDToBalanceEntry...)
	key = append(key, hodlerPKID[:]...)
	key = append(key, creatorPKID[:]...)
	return key
}
func _dbKeyForCreatorPKIDHODLerPKIDToBalanceEntry(creatorPKID *types.PKID, hodlerPKID *types.PKID) []byte {
...
func DBGetCreatorCoinBalanceEntryForHODLerAndCreatorPKIDsWithTxn(
	txn *badger.Txn, hodlerPKID *types.PKID, creatorPKID *types.PKID) *block_view.BalanceEntry {

	key := _dbKeyForHODLerPKIDCreatorPKIDToBalanceEntry(hodlerPKID, creatorPKID)
	balanceEntryObj := &block_view.BalanceEntry{}
	...
	if err != nil {
		glog.Errorf("DBGetCreatorCoinBalanceEntryForHODLerAndCreatorPubKeysWithTxn: Problem reading "+
			...
			types.PkToStringBoth(hodlerPKID[:]), types.PkToStringBoth(creatorPKID[:]))
		return nil
	}
...
func DBGetCreatorCoinBalanceEntryForHODLerAndCreatorPKIDs(
	handle *badger.DB, hodlerPKID *types.PKID, creatorPKID *types.PKID) *block_view.BalanceEntry {

	var ret *block_view.BalanceEntry
	handle.View(func(txn *badger.Txn) error {
...
func DBGetCreatorCoinBalanceEntryForCreatorPKIDAndHODLerPubKeyWithTxn(
	txn *badger.Txn, creatorPKID *types.PKID, hodlerPKID *types.PKID) *block_view.BalanceEntry {
...
func DBDeleteCreatorCoinBalanceEntryMappingsWithTxn(
	txn *badger.Txn, hodlerPKID *types.PKID, creatorPKID *types.PKID,
	params *types.DeSoParams) error {
...
func DBDeleteCreatorCoinBalanceEntryMappings(
	handle *badger.DB, hodlerPKID *types.PKID, creatorPKID *types.PKID,
	params *types.DeSoParams) error {
...
func DBPutCreatorCoinBalanceEntryMappingsWithTxn(
	txn *badger.Txn, balanceEntry *block_view.BalanceEntry,
	params *types.DeSoParams) error {
	...
	if err := txn.Set(_dbKeyForHODLerPKIDCreatorPKIDToBalanceEntry(
		...
		return errors.Wrapf(err, "DbPutCreatorCoinBalanceEntryMappingsWithTxn: Problem "+
			...
			types.PkToStringBoth(balanceEntry.HODLerPKID[:]),
			types.PkToStringBoth(balanceEntry.CreatorPKID[:]))
	}
...
func DBPutCreatorCoinBalanceEntryMappings(
	handle *badger.DB, balanceEntry *block_view.BalanceEntry, params *types.DeSoParams) error {
...
func GetSingleBalanceEntryFromPublicKeys(holder []byte, creator []byte, utxoView *block_view.UtxoView) (*block_view.BalanceEntry, error) {
	...
	balanceEntryMapKey := block_view.BalanceEntryMapKey{HODLerPKID: *holderPKID, CreatorPKID: *creatorPKID}
...
func DbGetBalanceEntry(db *badger.DB, holder *types.PKID, creator *types.PKID) *block_view.BalanceEntry {
	var ret *block_view.BalanceEntry
	db.View(func(txn *badger.Txn) error {
...
func DbGetHolderPKIDCreatorPKIDToBalanceEntryWithTxn(txn *badger.Txn, holder *types.PKID, creator *types.PKID) *block_view.BalanceEntry {
	...
	if err != nil {
		glog.Errorf("DbGetReposterPubKeyRepostedPostHashToRepostedPostMappingWithTxn: Problem decoding "+
			"balance entry for holder %v and creator %v", types.PkToStringMainnet(types.PKIDToPublicKey(holder)), types.PkToStringMainnet(types.PKIDToPublicKey(creator)))
		return nil
	}
	return balanceEntryObj
}
...
func DbGetBalanceEntriesYouHold(db *badger.DB, pkid *types.PKID, filterOutZeroBalances bool) ([]*block_view.BalanceEntry, error) {
	// Get the balance entries for the coins that *you hold*
	balanceEntriesYouHodl := []*block_view.BalanceEntry{}
	{
		...
		for _, byteString := range entryByteStringsFound {
			currentEntry := &block_view.BalanceEntry{}
			gob.NewDecoder(bytes.NewReader(byteString)).Decode(currentEntry)
...
func DbGetBalanceEntriesHodlingYou(db *badger.DB, pkid *types.PKID, filterOutZeroBalances bool) ([]*block_view.BalanceEntry, error) {
	// Get the balance entries for the coins that *hold you*
	balanceEntriesThatHodlYou := []*block_view.BalanceEntry{}
	{
...
func DBGetPaginatedPostsOrderedByTime(
	db *badger.DB, startPostTimestampNanos uint64, startPostHash *types.BlockHash,
	numToFetch int, fetchPostEntries bool, reverse bool) (
	_postHashes []*types.BlockHash, _tstampNanos []uint64, _postEntries []*block_view.PostEntry,
	_err error) {
	...
	postIndexKeys, _, err := DBGetPaginatedKeysAndValuesForPrefix(
		...
		len(_PrefixTstampNanosPostHash)+len(maxUint64Tstamp)+types.HashSizeBytes, /*keyLen*/
	...
	var postEntries []*block_view.PostEntry
	if fetchPostEntries {
...
func DBGetProfilesByUsernamePrefixAndDeSoLocked(
	db *badger.DB, usernamePrefix string, utxoView *block_view.UtxoView) (
	_profileEntries []*block_view.ProfileEntry, _err error) {
	...
	pubKeysMap := make(map[block_view.PkMapKey][]byte)
	for _, pkidBytes := range pkidsFound {
		...
		if len(pubKey) != 0 {
			pubKeysMap[block_view.MakePkMapKey(pubKey)] = pubKey
		}
	}
	...
	for username, profileEntry := range utxoView.ProfileUsernameToProfileEntry {
		if strings.HasPrefix(string(username[:]), lowercaseUsernamePrefixString) {
			pkMapKey := block_view.MakePkMapKey(profileEntry.PublicKey)
			pubKeysMap[pkMapKey] = profileEntry.PublicKey
		}
	}

	// Sigh.. convert the public keys *back* into PKIDs...
	profilesFound := []*block_view.ProfileEntry{}
...
func DBGetPaginatedProfilesByDeSoLocked(
	...
	_profilePublicKeys [][]byte, _profileEntries []*block_view.ProfileEntry, _err error) {
	...
	var profileEntries []*block_view.ProfileEntry
...
func _dbKeyForMempoolTxn(mempoolTx *lib.MempoolTx) []byte {
	// Make a copy to avoid multiple calls to this function re-using the same slice.
...
func DbPutMempoolTxnWithTxn(txn *badger.Txn, mempoolTx *lib.MempoolTx) error {
...
func DbPutMempoolTxn(handle *badger.DB, mempoolTx *lib.MempoolTx) error {
...
func DbGetMempoolTxnWithTxn(txn *badger.Txn, mempoolTx *lib.MempoolTx) *network.MsgDeSoTxn {

	mempoolTxnObj := &network.MsgDeSoTxn{}
...
func DbGetMempoolTxn(db *badger.DB, mempoolTx *lib.MempoolTx) *network.MsgDeSoTxn {
	var ret *network.MsgDeSoTxn
	db.View(func(txn *badger.Txn) error {
...
func DbGetAllMempoolTxnsSortedByTimeAdded(handle *badger.DB) (_mempoolTxns []*network.MsgDeSoTxn, _error error) {
	_, valuesFound := _enumerateKeysForPrefix(handle, _PrefixMempoolTxnHashToMsgDeSoTxn)

	mempoolTxns := []*network.MsgDeSoTxn{}
	for _, mempoolTxnBytes := range valuesFound {
		mempoolTxn := &network.MsgDeSoTxn{}
...
func FlushMempoolToDbWithTxn(txn *badger.Txn, allTxns []*lib.MempoolTx) error {
...
func FlushMempoolToDb(handle *badger.DB, allTxns []*lib.MempoolTx) error {
	err := handle.Update(func(txn *badger.Txn) error {
...
func DbDeleteMempoolTxnWithTxn(txn *badger.Txn, mempoolTx *lib.MempoolTx) error {
...
func DbDeleteMempoolTxn(handle *badger.DB, mempoolTx *lib.MempoolTx) error {
	return handle.Update(func(txn *badger.Txn) error {
